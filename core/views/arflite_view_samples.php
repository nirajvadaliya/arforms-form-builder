...
if ( $arf_samples == '' ) {
	...
	echo "<div class='error_message smpl-listing-error'>" .  __( 'Forms listing is currently unavailable. Please try again later.', 'arforms-form-builder' )  . '</div>';
		...
		foreach ( $arf_samples as $arf_sample ) {
			...
			<div class="arf_sample_container">
				<div class="arf_sample_title">
					<a href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" ><?php echo esc_html( $arf_sample['name'] ); ?></a>
				</div>
			...
			if ( $load_list_into_new_form_popup == false ) {
				...
				<div class="arf_sample_form_add_more">
					<a class="arf_sample_download_button" href="javascript: void(0);" data-arf-form-id="<?php echo esc_attr($arf_sample['form_id']); ?>"><?php _e( 'Install', 'arforms-form-builder' ); ?></a>
				</div>
			<?php } ?>
...
<div id="error_message" class="arf_error_message">
	<div class="message_descripiton">
		<div id="arf_sample_download_error" id=""><?php echo  __( 'Form is not available.', 'arforms-form-builder' ) ; ?></div>
