...
if ( is_array( $field ) ) {
	...
	?>
		<div class="sortable_inner_wrapper edit_field_type_<?php echo esc_attr($display['type']); ?>" id="arfmainfieldid_<?php echo esc_attr($field['id']); ?>" inner_class="<?php echo isset( $field['inner_class'] ) ? esc_attr($field['inner_class']) : 'arf_1col'; ?>" <?php echo $sortable_inner_field_style; ?>>
			<div id="arf_field_<?php echo esc_attr($field['id']); ?>" class="arfformfield control-group arfmainformfield   <?php echo isset( $newarr['position'] ) ? esc_attr($newarr['position']) . '_container' : 'top_container'; ?> <?php echo ( isset( $newarr['hide_labels'] ) && $newarr['hide_labels'] == 1 ) ? 'none_container' : ''; ?> arf_field_<?php echo esc_attr($field['id']); ?> ">
					...
					<div class="fieldname">
						...
						<label class="arf_main_label <?php echo esc_attr($arf_main_label_cls); ?>" id="field_<?php echo esc_attr($field['id']); ?>">
							<span class="arfeditorfieldopt_label arf_edit_in_place">
								<input type="text" class="arf_edit_in_place_input inplace_field" data-ajax="false" data-field-opt-change="true" data-field-opt-key='name' value="<?php echo htmlspecialchars( $field['name'] ); ?>" data-field-id="<?php echo esc_attr($field['id']); ?>" />
							</span>
							<?php if ( $is_required_field ) { ?>
							<span id="require_field_<?php echo esc_attr($field['id']); ?>">
								<a href="javascript:void(0)" onclick="javascript:arflitemakerequiredfieldfunction(<?php echo esc_attr($field['id']); ?>,<?php echo $field_required = ( isset( $field['required'] ) && $field['required'] == '0' ) ? '0' : '1'; ?>,'1')" class="arfaction_icon arfhelptip arffieldrequiredicon alignleft arfcheckrequiredfield<?php echo esc_attr($field_required); ?>" id="req_field_<?php echo esc_attr($field['id']); ?>" title="<?php echo  __( 'Click to mark as', 'arforms-form-builder' )  . ( $field['required'] == '0' ? ' ' : ' not ' ) .  __( 'compulsory field.', 'arforms-form-builder' ) ; ?>"></a>
							</span>
							<?php } ?>
						</label>
				...
				<div class="arf_fieldiconbox <?php echo ( $is_edit_option_icon ) ? 'arf_fieldiconbox_with_edit_option' : ''; ?>" data-field_id="<?php echo esc_attr($field['id']); ?>">
					<?php if ( $field['type'] != 'hidden' ) { ?>
						...
						if ( in_array( $field['type'], $fields_for_edit_options ) ) {
							echo $arflitefieldcontroller->arflite_get_field_control_icons( 'edit_options', '', $field['id'] );
						}
						if ( $field['type'] == 'html' ) {
							echo $arflitefieldcontroller->arflite_get_field_control_icons( 'running_total_icon' );
						}
						...
						if ( $field['type'] != 'html' && $field['type'] != 'arfslider' ) {
							...
							if ( $display['required'] && $field['type'] != 'arfslider' && $arf_disply_required_field ) {
								echo $arf_field_require_option_icon = $arflitefieldcontroller->arflite_get_field_control_icons( 'require', $field_required_cls, $field['id'], $field_required );
							}
						}
						?>
					...
					echo $arf_field_require_option_icon = $arflitefieldcontroller->arflite_get_field_control_icons( 'duplicate', '', $field['id'], 0, $field['type'], $id );
					echo $arf_field_require_option_icon = $arflitefieldcontroller->arflite_get_field_control_icons( 'delete', '', $field['id'], 0, '', '' );
					if ( $field['type'] != 'hidden' ) {

						echo $arflitefieldcontroller->arflite_get_field_control_icons( 'options', '', $field['id'], 0, $field['type'] );

						echo $arflitefieldcontroller->arflite_get_field_control_icons( 'move' );
					}
					?>
				</div>
					<?php
				...
				<?php } ?>
					...
					switch ( $field['type'] ) {
						...
						case 'image':
							...
							if ( $frm_css['arfinputstyle'] != 'material' ) {
								echo wp_kses( $prefix_suffix_wrapper_start, array( 'div' => array( 'id' => array(), 'class' => array() ) ) );
								echo wp_kses( $arf_prefix_icon, array( 'span' => array( 'class' => array() ), 'i' => array( 'class' => array() ) ) );
							}
							...
							?>
								...
								<input id="field_<?php echo esc_attr($field['field_key']); ?>" name="item_meta[<?php echo esc_attr($field['id']); ?>]" <?php echo isset( $field_opts['default_value'] ) && $field_opts['default_value'] != '' ? 'value="' . esc_attr($field_opts['default_value']) . '"' : ''; ?> <?php echo ( $placeholder_text != '' ) ? 'placeholder="' .esc_attr($placeholder_text) . '"' : ''; ?> type="text" <?php echo $default_country_code; ?> class="<?php echo esc_attr($input_cls) . ' ' . esc_attr($inp_cls); ?>" />
								<?php
								...
								if ( $frm_css['arfinputstyle'] != 'material' ) {
									echo wp_kses( $arf_suffix_icon, array( 'span' => array( 'class' => array() ), 'i' => array( 'class' => array() ) ) );
								...
								if ( isset( $field['options'] ) && ! empty( $field['options'] ) ) {
									...
									foreach ( $field['options'] as $opt_key => $opt ) {
										...
										if ( ! isset( $atts ) or ! isset( $atts['label'] ) or $atts['label'] ) {
											...
											echo '<input type="checkbox" name="' . esc_attr($field_name) . '[]" id="field_' . esc_attr($field['id']) . '-' . esc_attr($opt_key) . '" value="' . esc_attr( $field_val ) . '" ' . $checked . ' style="' . esc_attr($arf_radio_box_hide) . '"';
											if ( $k == 0 ) {
											...
											if ( $field_opts->use_image == 1 && $label_image != '' ) {
												...
												echo '<label for="field_'.$field['id'].'-'.$opt_key.'" class="arf_checkbox_label_image_editor '.esc_attr($temp_check).' '.$chk_icon .'">';
									...
									foreach ( $field['options'] as $opt_key => $opt ) {
										...
										if ( ! isset( $atts ) or ! isset( $atts['label'] ) or $atts['label'] ) {
											...
											if ( $field_opts->use_image == 1 && $label_image != '' ) {
												...
												echo '<span class="arf_radio_label_image_editor ' . esc_attr($checked) . ' '. $chk_icon.'">';
						...
						case 'select':
                            ...
                            echo $arflitemaincontroller->arflite_selectpicker_dom( $field_name, 'field_'.$field['field_key'], ' arf_form_field_picker ', '', $arfdefault_selected_val, $select_attrs, $select_field_opts, false, $opt_cls, false, array(), true, $field, false, '', '', $arf_set_label );
						...
						case 'captcha':
							...
							if ( empty( $arflitesettings->pubkey ) && ( isset( $field['is_recaptcha'] ) && $field['is_recaptcha'] != 'custom-captcha' ) ) {
								?>
												<div class="howto" id="setup_captcha_message" ><?php echo  __( 'Please setup site key and private key in Global Settings otherwise recaptcha will not appear', 'arforms-form-builder' ) ; ?></div>    <?php } ?>
						...
						case 'html':
							?>
							<p class="howto clear"><?php echo  __( 'Note: Set your custom html content', 'arforms-form-builder' ) ; ?></p>
							<?php
							break;
						case 'hidden':
							?>
							...
							<p class="howto clear"><?php echo  __( 'Note: This field will not show in the form. Enter the value to be hidden.', 'arforms-form-builder' ) ; ?><br/>
				...
				?>
				<input type="hidden" name="arf_field_data_<?php echo esc_attr($field['id']); ?>" id="arf_field_data_<?php echo esc_attr($field['id']); ?>" class="arf_field_data_hidden" value="<?php echo htmlspecialchars( json_encode( $new_field_obj ) ); ?>" data-field_options='<?php echo json_encode( $field_order ); ?>' />
				<div class="arf_field_option_model arf_field_option_model_cloned" data-field_id="<?php echo esc_attr($field['id']); ?>">
					<div class="arf_field_option_model_header"><?php echo  __( 'Field Options', 'arforms-form-builder' ) ; ?>&nbsp;<span class="arf_pre_populated_field_type" id="{arf_field_type}">[Field Type : [arf_field_type]]</span>&nbsp;<span class="arf_pre_populated_field_id" id="{arf_field_id}">[Field ID:[arf_field_id]]</span></div>
					<div class="arf_field_option_model_container">
						<div class="arf_field_option_content_row">
						</div>
					</div>
					<div class="arf_field_option_model_footer">
						<button type="button" class="arf_field_option_close_button" onClick="arflite_close_field_option_popup(<?php echo esc_attr($field['id']); ?>);"><?php echo  __( 'Cancel', 'arforms-form-builder' ) ; ?></button>
						<button type="button" class="arf_field_option_submit_button" data-field_id="<?php echo esc_attr($field['id']); ?>"><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
					</div>
				</div>
				<?php
				if ( in_array( $field['type'], $fields_for_edit_options ) ) {
					...
					<div class="arf_field_values_model" id="arf_field_values_model_skeleton_<?php echo esc_attr($field['id']); ?>">
						<div class="arf_field_values_model_header"><?php echo  __( 'Edit Options', 'arforms-form-builder' ) ; ?></div>
						...
						<div class="arf_field_values_model_footer">
							<button type="button" class="arf_field_values_close_button"><?php echo  __( 'Cancel', 'arforms-form-builder' ) ; ?></button>
							<button type="button" class="arf_field_values_submit_button" data-field-id="<?php echo esc_attr($field['id']); ?>"><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
						</div>
					</div>
					<?php
				}
	...
	if ( ! empty( $confirm_field_options ) ) {
		...
		if ( $confirm_field_options['type'] == 'email' ) {
			...
			if ( $frm_css['arfinputstyle'] != 'material' ) {
				?>
				<div id="arfmainfieldid_<?php echo esc_attr($field); ?>" class="sortable_inner_wrapper arf_confirm_field ui-droppable ui-sortable"  inner_class="<?php echo esc_attr($define_classes); ?>" <?php echo $sortable_inner_field_style; ?>>
					<div id="arf_field_<?php echo esc_attr($field); ?>" class="arfformfield control-group arfmainformfield <?php echo isset( $newarr['position'] ) ? esc_attr($newarr['position']) . '_container' : 'top_container'; ?> arf_field arf_confirm_field">
						...
						<div class="arf_fieldiconbox" data-field_id=<?php echo esc_attr($field); ?>>
							<div class='arf_field_option_icon'><a class='arf_field_option_input'><svg id='moveing' height='20' width='21'><g><?php echo ARFLITE_CUSTOM_MOVING_ICON; ?></g></svg></a></div>
						</div>
						<?php
		...
		?>
		<div class='sortable_inner_wrapper' inner_class='<?php echo esc_attr($define_classes); ?>' <?php echo $sortable_inner_field_style; ?>></div>
		<?php
