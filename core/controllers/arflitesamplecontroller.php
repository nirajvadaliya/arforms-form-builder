	...
	function arflite_samples_list( $load_list_into_new_form_popup = false ) {
		...
		if ( false == $sample_list_data ) {
			...
			if ( is_wp_error( $raw_response ) || $raw_response['response']['code'] != 200 ) {
				...
				echo "<div class='error_message sample_list_error-msg'>" .  __( 'Forms listing is currently unavailable. Please try again later.', 'arforms-form-builder' )  . '</div>';
			} else {

				set_transient( 'arflite_sample_listing_page', $raw_response['body'], DAY_IN_SECONDS );
				echo $arflitesamplecontroller->arflite_sample_display_forms( $raw_response['body'], $load_list_into_new_form_popup );
			}
		} else {
			echo $arflitesamplecontroller->arflite_sample_display_forms( $sample_list_data, $load_list_into_new_form_popup );
		}
	}
