...
<html <?php language_attributes(); ?>>
					...
					if ( $field['size'] != 1 ) {
						if ( isset( $newarr ) && $newarr['auto_width'] != '1' ) {
							...
							if ( isset( $field['field_width'] ) && $field['field_width'] != '' ) {
								...
								echo '.arflite_main_div_' . $field['form_id'] . ' .select_controll_' . $field['id'] . ':not([class*="span"]):not([class*="col-"]):not([class*="form-control"]){width:' . $field['field_width'] . 'px !important;}';
							}
						}
					}
				} elseif ( $field['type'] == 'time' ) {
					if ( isset( $field['field_width'] ) && $field['field_width'] != '' ) {
						echo '.arflite_main_div_' . $field['form_id'] . ' .time_controll_' . $field['id'] . ':not([class*="span"]):not([class*="col-"]):not([class*="form-control"]){width:' . $field['field_width'] . 'px !important;}';
					}
				}

				if ( isset( $field['field_width'] ) && $field['field_width'] != '' ) {
					echo ' .arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .help-block { width: ' . $field['field_width'] . 'px; } ';
				}
				...
				foreach ( $custom_css_array as $custom_css_block => $custom_css_classes ) {
					if ( isset( $field[ $custom_css_block ] ) && $field[ $custom_css_block ] != '' ) {
						...
						if ( $custom_css_block == 'css_outer_wrapper' ) {
							echo ' .arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container { ' . $field[ $custom_css_block ] . ' } ';
						} elseif ( $custom_css_block == 'css_outer_wrapper' ) {
							echo ' .arflite_main_div_' . $form->id . ' #heading_' . $field['id'] . ' { ' . $field[ $custom_css_block ] . ' } ';
						} elseif ( $custom_css_block == 'css_label' ) {
							echo ' .arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container label.arf_main_label { ' . $field[ $custom_css_block ] . ' } ';
						} elseif ( $custom_css_block == 'css_label' ) {
							echo ' .arflite_main_div_' . $form->id . ' #heading_' . $field['id'] . ' h2.arf_sec_heading_field { ' . $field[ $custom_css_block ] . ' } ';
						} elseif ( $custom_css_block == 'css_input_element' ) {

							if ( $field['type'] == 'textarea' ) {
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls textarea { ' . $field[ $custom_css_block ] . ' } ';
							} elseif ( $field['type'] == 'select' ) {
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls select { ' . $field[ $custom_css_block ] . ' } ';
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls .arfbtn.dropdown-toggle { ' . $field[ $custom_css_block ] . ' } ';
							} elseif ( $field['type'] == 'radio' ) {
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .arf_radiobutton label { ' . $field[ $custom_css_block ] . ' } ';
							} elseif ( $field['type'] == 'checkbox' ) {
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .arf_checkbox_style label { ' . $field[ $custom_css_block ] . ' } ';
							} else {
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls input { ' . $field[ $custom_css_block ] . ' } ';
								if ( $field['type'] == 'email' ) {
									echo '.arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container + .confirm_email_container .controls input {' . $field[ $custom_css_block ] . '}';
									echo ' .arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container + .confirm_email_container .arf_prefix_suffix_wrapper{ ' . $field[ $custom_css_block ] . ' }';
								}
								
								echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .arf_prefix_suffix_wrapper { ' . $field[ $custom_css_block ] . ' } ';
							}
						} elseif ( $custom_css_block == 'css_description' ) {
							echo ' .arflite_main_div_' . $form->id . '  #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .arf_field_description { ' . $field[ $custom_css_block ] . ' } ';
						} elseif ( $custom_css_block == 'css_description' ) {
							echo ' .arflite_main_div_' . $form->id . '  #heading_' . $field['id'] . ' .arf_heading_description { ' . $field[ $custom_css_block ] . ' } ';
						} elseif ( $custom_css_block == 'css_add_icon' ) {
							echo '.arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .arf_prefix,
                            .arflite_main_div_' . $form->id . ' #arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .arf_suffix { ' . $field[ $custom_css_block ] . ' } ';
							if ( $field['type'] == 'email' ) {
								echo '.arflite_main_div_' . $form->id . ' .arf_confirm_email_field_' . $field['id'] . ' .arf_prefix,
	...
	<body id="arf_preview_body" class="arf_preview_modal_body <?php echo ( is_rtl() ) ? 'arf_preview_rtl' : ''; ?>" >
		...
		if ( isset( $form->id ) && ! isset( $_REQUEST['form_id'] ) ) {
			$res = $wpdb->get_results( $wpdb->prepare( 'SELECT options FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $form->id ), 'ARRAY_A' );
		}
		if ( isset( $res ) ) {
			$res = $res[0];
		}
		...
		<div id="arfdevicebody" class="arfdevicecomputer arflite_preview_devicebody" align="center">
			...
			if ( $opt_id != '' ) {
				...
				echo $contents = apply_filters( 'arflite_pre_display_arfomrms', $contents, $form_id, $key );
			} else {
				echo  __( 'Please select valid forms', 'arforms-form-builder' ) ;
			}
			?>
		</div>
		<?php
