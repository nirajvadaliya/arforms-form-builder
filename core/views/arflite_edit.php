...
if ( $action == 'duplicate' || $action == 'edit' ) {
	$record = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $id ) );
}

if ( isset( $record ) && $record->is_template && sanitize_text_field( $_REQUEST['arfaction'] ) != 'duplicate' ) {
	wp_die(  __( 'That template cannot be edited', 'arforms-form-builder' ) ) ;
}
if ( ! isset( $record ) ) {
	$record = new stdClass();
}
...
if ( ! empty( $record_arr ) ) {
	...
	$all_fields            = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE form_id = %d ORDER BY ID ASC', $form_id ) );
}
...
foreach ( $form_defaults as $opt => $default ) {
	...
	if ( ! isset( $values[ $opt ] ) || $values[ $opt ] == '' ) {
		if ( $opt == 'notification' ) {
			$values[ $opt ] = ( $_POST && isset( $_POST[ $opt ] ) ) ? $_POST[ $opt ] : $default;
			foreach ( $default as $o => $d ) {
				if ( $o == 'email_to' ) {
					$d = '';
				}
				$values[ $opt ][0][ $o ] = ( $_POST && isset( $_POST[ $opt ][0][ $o ] ) ) ? $_POST[ $opt ][0][ $o ] : $d;
				unset( $o );
				unset( $d );
			}
		} else {
...
$upload_main_url = ARFLITE_UPLOAD_URL . '/maincss';
	...
	echo wp_kses( $arf_form_style, array( 'style' => array( 'type' => array(), 'class' => array() ) ) );
...
<input type="hidden" id="arflite_browser_info" value='<?php echo json_encode( $browser_info ); ?>' />
<input type="hidden" id="arflite_skin_json" value="<?php echo base64_encode( json_encode( $skinJson ) ); ?>" />

<style type="text/css" id="arf_form_other_css_<?php echo esc_attr($id); ?>">
	<?php
...
<style type="text/css" id='arf_form_<?php echo esc_attr($id); ?>'>
<?php
		...
		if ( $custom_css_block_form == 'arf_form_outer_wrapper' ) {
			...
			foreach ( $arf_form_outer_wrapper_array as $arf_form_outer_wrapper1 ) {
				if ( $arf_form_outer_wrapper1 == '.arf_form_outer_wrapper' ) {
					echo '.arflite_main_div_' . $form->id . '.arf_form_outer_wrapper { ' . $form->options[ $custom_css_block_form ] . ' } ';
				}
			...
			foreach ( $arf_form_inner_wrapper_array as $arf_form_inner_wrapper1 ) {
				if ( $arf_form_inner_wrapper1 == '.arf_fieldset' ) {
					echo '.arflite_main_div_' . $form->id . ' ' . $arf_form_inner_wrapper1 . ' { ' . $form->options[ $custom_css_block_form ] . ' } ';
				}
				if ( $arf_form_inner_wrapper1 == '.arfmodal' ) {
					echo '.arfmodal .arfmodal-body .arflite_main_div_' . $form->id . ' .arf_fieldset { ' . $form->options[ $custom_css_block_form ] . ' } ';
				}
			}
			...
			foreach ( $arf_form_error_message_array as $arf_form_error_message1 ) {
				echo '.arflite_main_div_' . $form->id . ' ' . $arf_form_error_message1 . ' { ' . $form->options[ $custom_css_block_form ] . ' } ';
			}
		} else {
			echo '.arflite_main_div_' . $form->id . ' ' . $custom_css_classes_form . ' { ' . $form->options[ $custom_css_block_form ] . ' } ';
		}
	}
}
...
<div class="arf_editor_wrapper">
	<div id="arfsaveformloader"><?php echo ARFLITE_LOADER_ICON; ?></div>
	<input type="hidden" id="arf_control_labels" value="" data-field-id="" />
	<input type="hidden" id="arf_reset_styling" value="false" />
	<input type="hidden" name="arflite_validation_nonce" id="arflite_validation_nonce" value="<?php echo wp_create_nonce( 'arflite_wp_nonce' ); ?>" />
	<input type="hidden" id="arflite_preset_nonce" value="<?php echo wp_create_nonce( 'arflite_wp_preset_nonce' ); ?>" />
	...
	<input type="hidden" name="arfgettemplate_style" id="arfgettemplate_style" value="<?php echo ( isset( $_GET['templete_style'] ) && $_GET['templete_style'] != '' ) ? esc_attr( $_GET['templete_style'] ) : ''; ?>" />
	...
	<form name="arf_form" id="frm_main_form" method="post" onSubmit='return arflitemainformedit(0);'>
		<input type="hidden" name="arfmainformurl" data-id="arfmainformurl" value="<?php echo ARFLITEURL; ?>" />
		...
		<input type="hidden" name="arfaction" id="arfaction" value="<?php echo sanitize_text_field( $_GET['arfaction'] ); ?>" />
		<input type="hidden" name="arfajaxurl" id="arfajaxurl" class="arf_ajax_url" value="<?php echo esc_attr( $arfliteajaxurl ); ?>" />
		<input type="hidden" name="arffiledragurl" data-id="arffiledragurl" value="<?php echo ARFLITE_FILEDRAG_SCRIPT_URL; ?>" />

		<input type="hidden" name="prev_arfaction" value="<?php sanitize_text_field( $_GET['arfaction'] ); ?>" />
		...
		<input type="hidden" id="arf_field_order" name="arf_field_order" value='<?php echo esc_attr($arf_field_order); ?>' data-db-field-order='<?php echo ( sanitize_text_field( $_GET['arfaction'] ) == 'edit' ) ? esc_attr($arf_field_order) : ''; ?>' />
		<input type="hidden" id="arf_inner_field_order" name="arf_inner_field_order" value='<?php echo esc_attr($arf_inner_field_order); ?>' data-db-field-order='<?php echo ( sanitize_text_field( $_GET['arfaction'] ) == 'edit' ) ? esc_attr($arf_inner_field_order) : ''; ?>' />
		<input type="hidden" id="arf_field_resize_width" name="arf_field_resize_width" value='<?php echo esc_attr($arf_field_resize_width); ?>' data-db-field-resize='<?php echo ( sanitize_text_field( $_GET['arfaction'] ) == 'edit' ) ? esc_attr($arf_field_resize_width) : ''; ?>' />
		<input type="hidden" id="arf_inner_field_resize_width" name="arf_inner_field_resize_width" value='<?php echo esc_attr($arf_inner_field_resize_width); ?>' data-db-field-resize='<?php echo ( sanitize_text_field( $_GET['arfaction'] ) == 'edit' ) ? esc_attr($arf_inner_field_resize_width) : ''; ?>' />

		<input type="hidden" id="arf_input_radius" name="arf_input_radius" value='<?php echo esc_attr($newarr['border_radius']); ?>' />
		<?php $browser_info = $arfliterecordcontroller->arflitegetBrowser( $_SERVER['HTTP_USER_AGENT'] ); ?>
			...
			<div class="arf_editor_header_inner_belt">
					...
					<ul class="arf_editor_top_menu">
						<li class="arf_editor_top_menu_item" id="mail_notification">
							...
							<label class="arf_editor_top_menu_label">
								<?php echo  __( 'Email Notifications', 'arforms-form-builder' ) ; ?>
							</label>
						</li>
						<li class="arf_editor_top_menu_item" id="conditional_law">
							...
							<label class="arf_editor_top_menu_label">
								<?php echo  __( 'Conditional Rule', 'arforms-form-builder' ) ; ?>
							</label>
						</li>
						<li class="arf_editor_top_menu_item" id="submit_action">
							...
							<label class="arf_editor_top_menu_label">
								<?php echo  __( 'Submit Action', 'arforms-form-builder' ) ; ?>
							</label>
						</li>
						<li class="arf_editor_top_menu_item" id="email_marketers">
							...
							<label class="arf_editor_top_menu_label">
								<?php echo  __( 'Opt-ins', 'arforms-form-builder' ) ; ?>
							</label>
						</li>
						<li class="arf_editor_top_menu_item arf_editor_top_menu_dropdown">
							...
							<label class="arf_editor_top_menu_label">
								<?php echo  __( 'Other Options', 'arforms-form-builder' ) ; ?>
							...
							<div class="arf_editor_top_dropdown_submenu_container">
								<ul class="arf_editor_top_dropdown">
									<li class="arf_editor_top_dropdown_option" id="general_options"><?php echo __( 'General Options', 'arforms-form-builder' ); ?></li>
									<li class="arf_editor_top_dropdown_option" id="arf_hidden_fields_options"><?php echo __( 'Hidden Input Fields', 'arforms-form-builder' ); ?></li>
									<li class="arf_editor_top_dropdown_option" id="arf_tracking_code"><?php echo  __( 'Submit Tracking Script', 'arforms-form-builder' ) ; ?></li>
									<li class="arf_editor_top_dropdown_option <?php echo ( sanitize_text_field( $_GET['arfaction'] ) == 'new' || sanitize_text_field( $_GET['arfaction'] ) == 'duplicate' ) ? 'arf_export_form_editor_note' : ''; ?>" id="arflite_export_current_form_link"><?php echo  __( 'Export Form', 'arforms-form-builder' ) ; ?></li>
				...
				<div class="arf_editor_top_menu_button_wrapper">
					<div class="arf_editor_shortcode_wrapper">
						<div class="arf_editor_shortcode_icon_wrapper arfbelttooltip" id="arf_shortcodes_info" data-title="<?php echo  __( 'Shortcodes', 'arforms-form-builder' ) ; ?>"></div>
						<div class="arf_editor_form_shortcode_list_popup">
							<div class="arf_editor_form_shortcode_list_content">
								...
								<ul id="arf_editor_saved_form_shortcodes" class="arf_editor_form_shortcode_list <?php echo esc_attr($arf_saved_form_shortcode); ?>">
									<li class="arf_editor_form_shortcode_header"><span><?php echo  __( 'Shortcodes', 'arforms-form-builder' ) ; ?></span></li>
									<li class="arf_editor_form_shortcode">
										<span class="arf_shortcode_label"><?php echo  __( 'Embed Inline Form', 'arforms-form-builder' ) ; ?></span>
										<span class="arf_shortcode_content">[ARFormslite id=<?php echo esc_html( $shortcode_form_id ); ?>]</span>
									</li>
									<li class="arf_editor_form_shortcode">
										<span class="arf_shortcode_label"><?php echo  __( 'PHP Function', 'arforms-form-builder' ) ; ?></span>
								...
								<ul id="arf_editor_unsaved_form_shortcodes" class="arf_editor_form_shortcode_list <?php echo esc_attr($arf_unsaved_form_shortcode); ?>">
									<li class="arf_editor_form_shortcode_header"><span><?php echo  __( 'Shortcodes', 'arforms-form-builder' ) ; ?></span></li>
									<li class="arf_editor_form_shortcode">
										<span class="arf_shortcode_content"><?php echo  __( 'Please save form to generate shortcode.', 'arforms-form-builder' ) ; ?></span>
									</li>
								</ul>
					...
					<button type="submit" name="arf_save" class="arf_top_menu_save_button rounded_button btn_green">
						<?php echo  __( 'Save', 'arforms-form-builder' ) ; ?>
					</button>
					<button type="button" name="arf_preview" class="arf_top_menu_preview_button arfbelttooltip" data-url="<?php echo ( $action == 'new' ) ? esc_attr($pre_link) . '&form_id=' . esc_attr($id) : esc_attr($pre_link); ?>" data-default-url="<?php echo ( $action == 'new' ) ? esc_attr($pre_link) : ''; ?>" onclick="arflitegetformpreview();" data-title="<?php echo  __( 'Preview', 'arforms-form-builder' ) ; ?>" >
					...
					<button type="button" name="arf_reset" class="arf_top_menu_reset_button arfbelttooltip" data-title="<?php echo  __( 'Reset Style', 'arforms-form-builder' ) ; ?>" onclick="arflite_reset_style_functionality();" >
					...
					<button type="button" name="arf_cancel" class="arf_top_menu_cancel_button arfbelttooltip" onClick="window.location = '<?php echo admin_url( 'admin.php?page=ARForms-Lite' ); ?>'" data-title="<?php echo  __( 'Cancel', 'arforms-form-builder' ) ; ?>">
		...
		<div class="arf_editor_header_shortcode_belt">
			...
			<div class="arf_editor_header_form_width">
				<div class="arf_editor_form_width_wrapper">
					<span class="arf_editor_form_width_label"><?php echo  __( 'Width', 'arforms-form-builder' ) ; ?></span>
					<span class="arfform_width_header_span" >
						<?php
                            ...
                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arf_editor_form_width_unit', 'arf_editor_form_width_unit', 'arfform_width_header_dl', 'width:40px;', $newarr['form_width_unit'], array(), $form_width_unit_opts );
                        ?>
					</span>
		...
		<div class="arf_form_editor_wrapper">
			<div class="arf_form_element_wrapper">
				...
				<ul class="arf_form_style_tabs">
					<li class="arf_form_element_type_tab_item active" data-id="arf_form_input_fields_container"><?php echo  __( 'Input Fields', 'arforms-form-builder' ) ; ?></li>
					<li class="arf_form_element_type_tab_item" data-id="arf_form_other_fields_container"><?php echo  __( 'Other Fields', 'arforms-form-builder' ) ; ?></li>
				</ul>
				<ul class="arf_form_elements_container active" id="arf_form_input_fields_container">
					...
					foreach ( $sortedFields as $key ) {
						if ( in_array( $key, $input_fields ) ) {
							...
							?>
							<li class="arf_form_element_item frmbutton <?php echo esc_attr($pro_class); ?> frm_t<?php echo esc_attr($key); ?>" id="<?php echo esc_attr($key); ?>" data-field-id="<?php echo esc_attr($id); ?>" data-type="<?php echo esc_attr($key); ?>">
								<div class="arf_form_element_item_inner_container" <?php echo ( in_array( $key, $arflite_pro_fields ) ) ? ' data-title="'.__( 'Pro', 'arforms-form-builder') . '" ' : ''; ?>>
				...
				<ul class="arf_form_elements_container" id="arf_form_other_fields_container">
					...
					foreach ( $sortedFields as $key ) {
						if ( in_array( $key, $other_fields ) ) {
							...
							?>
								<li class="arf_form_element_item <?php echo esc_attr($full_width_cls); ?> frmbutton frm_t<?php echo esc_attr($key); ?>" id="<?php echo esc_attr($key); ?>" data-field-id="<?php echo esc_attr($id); ?>" data-type="<?php echo esc_attr($key); ?>" >
									<div class="arf_form_element_item_inner_container" <?php echo ( in_array( $key, $arflite_pro_fields ) ) ? ' data-title="' . __( 'Pro', 'arforms-form-builder' ) .'" ' : ''; ?>>
										<span class="arf_form_element_item_icon">
										<?php
											echo wp_kses(
		...
		<button type="button" class="arf_hide_form_element_wrapper"><svg viewBox="<?php echo esc_attr($viewBox); ?>" width="25px" height="45px"><path xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" fill="#4E5462" d="M3.845,6.872l4.816,4.908l-1.634,1.604L0.615,6.849L0.625,6.84  L0.617,6.832L7.152,0.42l1.603,1.634L3.845,6.872z"/></svg></button>
			...
			<?php echo str_replace( 'id="{arf_id}"', 'id="arfeditor_loader" ', ARFLITE_LOADER_ICON ); ?>
			...
			<div class="arf_form_styling_tools">
				<ul class="arf_form_style_tabs">
					<li class="arf_form_style_tab_item active" data-id="arf_form_styling_tools"><?php echo  __( 'Style Options', 'arforms-form-builder' ) ; ?></li>
					<li class="arf_form_style_tab_item" data-id="arf_form_custom_css"><?php echo  __( 'Custom CSS', 'arforms-form-builder' ) ; ?></li>
				</ul>
				...
				<div class="arf_form_style_tab_container active" id="arf_form_styling_tools">
					<input type="hidden" name="arfmf" value="<?php echo esc_attr($id); ?>" id="arfmainformid" />
					<div class="arf_form_style_tab_accordion">
						<div class="arf_form_accordion_tabs">
							<dl class="arf_accordion_tab_color_options active">
								<dd>
									<a href="javascript:void(0)" data-target="arf_accordion_tab_color_options"><?php echo  __( 'Basic Styling Options', 'arforms-form-builder' ) ; ?></a>
									<div class="arf_accordion_container active">
										<div class="arf_input_style_container">
											<div class="arf_accordion_container_row arf_padding">
												<div class='arf_accordion_outer_title'><?php echo  __( 'Select Theme', 'arforms-form-builder' ) ; ?></div>
											</div>
											<div class="arf_accordion_container_row">
												<div class='arf_accordion_inner_title arf_width_50'><?php echo __( 'Input Style', 'arforms-form-builder' ); ?></div>
												<div class="arf_accordion_content_container arf_width_50 arf_right arfinputstyledrpdwn_container">
														...
														<?php
															...
															echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfinpst', 'arfmainforminputstyle', '', '', $newarr['arfinputstyle'], array(), $inputStyle, false, $arfmainforminputstyle_options_cls, false, array(), false, array(), false, 'arf_input_style_dpdn_ul');
														?>
												</div>
											</div>
										...
										<div class="arf_color_scheme_container">
											<div class="arf_accordion_container_row arf_padding">
												<div class='arf_accordion_outer_title'><?php echo  __( 'Color Scheme', 'arforms-form-builder' ) ; ?></div>
											</div>
											<div class="arf_accordion_container_row">
												<div class='arf_accordion_inner_title arf_custom_color_title'><?php echo  __( 'Choose Color', 'arforms-form-builder' ) ; ?></div>
												...
												<div class="arf_customize_color_div arf_right">
													<div class="arf_customize_color_inner_label_div">
														<div class='arf_accordion_inner_title arf_label_custom_color arf_width_90'><?php echo  __( 'Custom Color', 'arforms-form-builder' ) ; ?></div>
													</div>
													<div class="arf_customize_color_inner_control_div">
														...
														<div class="arf_custom_color">
															...
															<div class="arf_custom_color_label" id="arf_custom_color_label"><?php echo  __( 'Custom', 'arforms-form-builder' ) ; ?>
															</div>
														</div>
													</div>
												</div>
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class='arf_accordion_outer_title'><?php echo  __( 'Font Options', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row">
											<div class='arf_accordion_inner_title arf_width_50'><?php echo  __( 'Font Family', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50 arf_right arfinputstyledrpdwn_container">
												<?php
                                                    $fontsarr = array(
                                                        '' => array(
                                                    ...
                                                    echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfcommonfont', 'arfcommonfontfamily', '','', $newarr['arfcommonfont'], array(), $fontsarr, true, array(), false, array(), false, array(), true );

                                                ?>
											</div>
										</div>
										<div class="arf_accordion_container_row arf_accordion_container_row_input_size" >
											<div class='arf_accordion_inner_title arfwidth40'><?php echo __( 'Input Field size', 'arforms-form-builder' ); ?></div>
											<div class="arf_accordion_content_container arfwidth60 arf_accordion_container_mar">
												<div class="arf_slider_wrapper">
													...
													<div class="arf_slider_unit_data">
														<div class="input-size-slider-start"><?php echo  __( '1', 'arforms-form-builder' ) ; ?></div>
														<div class="input-size-slider-end"><?php echo  __( '10', 'arforms-form-builder' ) ; ?></div>
													</div>
											...
											<div class="arf_right arfmarginright">
												...
												<div class="arf_custom_font arf_basic_cutom_font_all">
													...
													<div class="arf_custom_font_label"><?php echo  __( 'Advanced font options', 'arforms-form-builder' ) ; ?>
													</div>
												</div>
											</div>
										</div>
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class='arf_accordion_outer_title'><?php echo  __( 'Form Width Settings', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Form Width', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container" style="<?php echo is_rtl() ? 'width:50%;' : 'width:58%;'; ?>" >
												<div class="arf_dropdown_wrapper">
                                                    <?php
                                                        ...
                                                        echo $arflitemaincontroller->arflite_selectpicker_dom( 'arffu', 'arffu', '', 'width:50px;', $newarr['form_width_unit'], $form_width_unit_attr, $form_width_unit_opts );
                                                    ?>
                                                </div>
												<input type="text" name="arffw" class="arf_small_width_txtbox arfcolor" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id}.arf_form_outer_wrapper~|~max-width{arf_form_width_unit}","material":".arflite_main_div_{arf_form_id}.arf_form_outer_wrapper~|~max-width{arf_form_width_unit}"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_outer_wrapper" value="<?php echo esc_attr( $newarr['arfmainformwidth'] ); ?>" id="arf_form_width"/>
											</div>
										...
										<div class="arf_accordion_container_row_separator"></div>
											<div class="arf_accordion_container_row arf_padding">
                                                <div class='arf_accordion_outer_title'><?php echo addslashes(esc_html__('Success/Error Message Position', 'arforms-form-builder')); ?></div>
                                            </div>
                                                <div class="arf_accordion_container_row arf_half_width">
                                                <div class="arf_accordion_inner_title"><?php echo addslashes(esc_html__('Position', 'arforms-form-builder')); ?></div>
                                                <div class="arf_accordion_content_container arf_align_right arf_right">                                                    
                                                    <div class="arf_toggle_button_group arf_two_button_group">
                                                        ...
                                                        <label class="arf_toggle_btn <?php echo ($newarr['arfsuccessmsgposition'] == 'bottom') ? 'arf_success' : ''; ?>">
                                                            <input type="radio" name="arfsuccessmsgposition" class="visuallyhidden" id="success_msg_position_bottom" value="bottom" <?Php checked($newarr['arfsuccessmsgposition'], 'bottom'); ?> /><?php echo addslashes(esc_html__('Bottom', 'arforms-form-builder')); ?>
                                                        </label>
                                                        <label class="arf_toggle_btn <?php echo ($newarr['arfsuccessmsgposition'] == 'top') ? 'arf_success' : ''; ?>">
                                                            <input type="radio" name="arfsuccessmsgposition" class="visuallyhidden" id="success_msg_position_top" value="top" <?Php checked($newarr['arfsuccessmsgposition'], 'top'); ?> /><?php echo addslashes(esc_html__('Top', 'arforms-form-builder')); ?>
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class='arf_accordion_outer_title'><?php echo  __( 'Validation Message Style', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_container_row arf_half_width">
												<div class="arf_accordion_inner_title"><?php echo  __( 'Type', 'arforms-form-builder' ) ; ?></div>
												<div class="arf_accordion_content_container arf_align_right arf_right arfinputstyledrpdwn_container">
													<div class="arf_toggle_button_group arf_two_button_group">
														...
														<label class="arf_toggle_btn <?php echo ( $newarr['arferrorstyle'] == 'normal' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arfest" class="visuallyhidden" id="arfest1" value="normal" <?php checked( $newarr['arferrorstyle'], 'normal' ); ?> /><?php echo  __( 'Standard', 'arforms-form-builder' ) ; ?></label>
														<label class="arf_toggle_btn <?php echo ( $newarr['arferrorstyle'] == 'advance' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arfest" class="visuallyhidden" id="arfest2" value="advance" <?php checked( $newarr['arferrorstyle'], 'advance' ); ?> /><?php echo  __( 'Modern', 'arforms-form-builder' ) ; ?></label>
													</div>
												</div>
											</div>
											<div class="arf_accordion_container_row arf_half_width" id="arf_validation_message_style_position" style="<?php echo ( $newarr['arferrorstyle'] == 'normal' ) ? 'display: none;' : ''; ?>">
												<div class="arf_accordion_inner_title" ><?php echo  __( 'Position', 'arforms-form-builder' ) ; ?></div>
												<div class="arf_accordion_content_container">
													<div class="arf_toggle_button_group arf_four_button_group">
														...
														<label class="arf_toggle_btn <?php echo ( $newarr['arferrorstyleposition'] == 'right' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arfestbc" class="visuallyhidden" data-id="arfestbc2" value="right" <?php checked( $newarr['arferrorstyleposition'], 'right' ); ?> /><?php echo  __( 'Right', 'arforms-form-builder' ) ; ?></label>
														<label class="arf_toggle_btn <?php echo ( $newarr['arferrorstyleposition'] == 'left' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arfestbc" class="visuallyhidden" data-id="arfestbc2" value="left" <?php checked( $newarr['arferrorstyleposition'], 'left' ); ?> /><?php echo  __( 'Left', 'arforms-form-builder' ) ; ?></label>
														<label class="arf_toggle_btn <?php echo ( $newarr['arferrorstyleposition'] == 'bottom' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arfestbc" class="visuallyhidden" data-id="arfestbc2" value="bottom" <?php checked( $newarr['arferrorstyleposition'], 'bottom' ); ?> /><?php echo  __( 'Bottom', 'arforms-form-builder' ) ; ?></label>
														<label class="arf_toggle_btn <?php echo ( $newarr['arferrorstyleposition'] == 'top' ) ? 'arf_success' : ''; ?>"><input type='radio' name='arfestbc' class='visuallyhidden' id='arfestbc1' value='top' <?php checked( $newarr['arferrorstyleposition'], 'top' ); ?> /><?php echo  __( 'Top', 'arforms-form-builder' ) ; ?></label>
													</div>
												</div>
											</div>
												...
												<div class="arf_accordion_content_container">
													<div class="arf_toggle_button_group arf_four_button_group">
														...
														<label class="arf_toggle_btn <?php echo ( $newarr['arfstandarderrposition'] == 'absolute' ) ? 'arf_success' : ''; ?>">
															<input type="radio" name="arfstndrerr" class="visuallyhidden" data-id="arfstndrerr2" value="absolute" <?php checked( $newarr['arfstandarderrposition'], 'absolute' ); ?> /><?php echo  __( 'Absolute', 'arforms-form-builder' ) ; ?>
														</label>

														<label class="arf_toggle_btn <?php echo ( $newarr['arfstandarderrposition'] == 'relative' ) ? 'arf_success' : ''; ?>">
															<input type="radio" name="arfstndrerr" class="visuallyhidden" data-id="arfstndrerr2" value="relative" <?php checked( $newarr['arfstandarderrposition'], 'relative' ); ?> /><?php echo  __( 'Relative', 'arforms-form-builder' ) ; ?>
														</label>
													</div>
												</div>
											</div>
							...
							<dl class="arf_accordion_tab_form_settings">
								<dd>
									<a href="javascript:void(0)" data-target="arf_accordion_tab_form_settings"><?php echo  __( 'Advanced Form Options', 'arforms-form-builder' ) ; ?></a>
									<div class="arf_accordion_container">
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Form Title options', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text arf_width_50"><?php echo  __( 'Display Title and Description', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_right arf_width_50">
												<div class="arf_float_right arfmarginright4">
													<label class="arf_js_switch_label">
														<span><?php echo  __( 'No', 'arforms-form-builder' ) ; ?>&nbsp;</span>
													</label>
													...
													<label class="arf_js_switch_label">
														<span>&nbsp;<?php echo  __( 'Yes', 'arforms-form-builder' ) ; ?></span>
													</label>
												</div>
											</div>
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo  __( 'Title Alignment', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_right arf_right arfinputstyledrpdwn_container">
										...
										<div class="arf_accordion_container_row arf_half_width">
											...
											<div class="arf_accordion_content_container arf_align_center arf_form_container arf_right arfformmarginvals arfinputstyledrpdwn_container">
												...
												<div class="arf_form_margin_box_wrapper"><input type="text" name="arfformtitlepaddingsetting_1" id="arfformtitlepaddingsetting_1" value="<?php echo esc_attr( $newarr['arfmainformtitlepaddingsetting_1'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-top||.arflite_main_div_{arf_form_id} .arftitlecontainer~|~margin-top","material":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-top"}' class="arf_form_margin_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_margin" /><br /><span class="arf_px arf_font_size arfformmarginleft"><?php echo  __( 'Top', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_form_margin_box_wrapper"><input type="text" name="arfformtitlepaddingsetting_2" id="arfformtitlepaddingsetting_2" value="<?php echo esc_attr( $newarr['arfmainformtitlepaddingsetting_2'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-right","material":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-right"}' class="arf_form_margin_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_margin" /><br /><span class="arf_px arf_font_size arfformmarginleft"><?php echo  __( 'Right', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_form_margin_box_wrapper"><input type="text" name="arfformtitlepaddingsetting_3" id="arfformtitlepaddingsetting_3" value="<?php echo esc_attr( $newarr['arfmainformtitlepaddingsetting_3'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-bottom||.arflite_main_div_{arf_form_id} .arftitlecontainer~|~margin-bottom","material":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-bottom"}' class="arf_form_margin_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_margin" /><br /><span class="arf_px arf_font_size arfformmarginleft"><?php echo  __( 'Bottom', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_form_margin_box_wrapper"><input type="text" name="arfformtitlepaddingsetting_4" id="arfformtitlepaddingsetting_4" value="<?php echo esc_attr( $newarr['arfmainformtitlepaddingsetting_4'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-left","material":".arflite_main_div_{arf_form_id} .allfields .arftitlediv~|~margin-left"}' class="arf_form_margin_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_margin" /><br /><span class="arf_px arf_font_size arfformmarginleft"><?php echo  __( 'Left', 'arforms-form-builder' ) ; ?></span></div>
											</div>
											<?php
											$arfformtitlepaddingsetting_value = '';
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class='arf_accordion_outer_title'><?php echo  __( 'Form Settings', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Form Alignment', 'arforms-form-builder' ) ; ?></div>
										...
										<div class="arf_accordion_container_row arf_half_width <?php echo esc_attr($arf_bg_position_height_style_x); ?>" id="arf_bg_img_position">
											<div class="arf_accordion_inner_title arf_bg_img_positionx-label arf_form_padding arf_two_row_text"><?php echo  __( 'Background Image Position-X', 'arforms-form-builder' ) ; ?></div>
                                                ...
                                                <div class="arf_dropdown_wrapper" style="width: 100%;">
                                                    <?php
                                                        ...
                                                        $bg_position_attr = array(
                                                            'onchange' => 'arflite_update_form_bg_position(this,"x", "arf_form_bg_position_input_div_x", "arf_fieldset_' . esc_js($id) . '")'
                                                        );

                                                        echo $arflitemaincontroller->arflite_selectpicker_dom( 'arf_bg_position_x', 'arf_bg_position_x', '', 'width:70%;margin-left: 15px;', $bg_position_selected_x, $bg_position_attr, $bg_position_opt );
                                                    ?>
                                                    <span class="arf_px arf_font_size arfpxspan" style="margin-right:0;position: relative;"><?php echo addslashes(esc_html__('X-axis', 'arforms-form-builder')); ?></span>
                                                </div>  
											...
											<div class="arf_form_bg_position_input_container">
												<div class="arf_form_bg_position_input_div" id="arf_form_bg_position_input_div_x" style="<?php echo esc_attr($arf_bg_position_style_x); ?>">
													<span class="arf_px arf_font_size arfpxspan arf_margin-right0"><?php echo  __( 'X-axis', 'arforms-form-builder'  ); ?></span>
													<input type="text" name="arf_bg_position_input_x" id="arf_form_bg_position_input_x" value="<?php echo ( isset( $newarr['arf_bg_position_input_x'] ) && $newarr['arf_bg_position_input_x'] != '' ) ? esc_attr( $newarr['arf_bg_position_input_x'] ) : ''; ?>" class="arf_form_bg_position_input" onfocusout="arflite_set_form_bg_position(this, 'x', 'arf_fieldset_<?php echo esc_js( $id ); ?>')">
												</div>
											</div>
										</div>
										...
										<div class="arf_accordion_container_row bg-img-posy-wrap arf_half_width <?php echo esc_attr($arf_bg_position_height_style_y); ?>">
											<div class="arf_accordion_inner_title arf_form_padding arf_two_row_text bg-img-posy-title"><?php echo  __( 'Background Image Position-Y', 'arforms-form-builder' ) ; ?></div>
											...
											<div class="arf_form_bg_position_container">
                                                <div class="arf_dropdown_wrapper" style="width: 100%;">
                                                    <?php
                                                        ...
                                                        $bg_position_attr = array(
                                                            'onchange' => 'arflite_update_form_bg_position(this,"y", "arf_form_bg_position_input_div_y", "arf_fieldset_' . esc_js($id) . '");'
                                                        );

                                                        echo $arflitemaincontroller->arflite_selectpicker_dom( 'arf_bg_position_y', 'arf_bg_position_y', '', 'width:70%;margin-left:15px;', $bg_position_selected_y, $bg_position_attr, $bg_position_opt );
                                                    ?>
                                                    <span class="arf_px arf_font_size arfpxspan" style="margin-right:0;position: relative;"><?php echo addslashes(esc_html__('Y-axis', 'arforms-form-builder')); ?></span>
                                                </div>
                                            </div>
											...
											<div class="arf_form_bg_position_input_container">
												<div class="arf_form_bg_position_input_div" id="arf_form_bg_position_input_div_y" style="<?php echo esc_attr($arf_bg_position_style_y); ?>">
													<span class="arf_px arf_font_size arfpxspan arf_margin-right0"><?php echo  __( 'Y-axis', 'arforms-form-builder' ) ; ?></span>
													<input type="text" name="arf_bg_position_input_y" id="arf_form_bg_position_input_y" value="<?php echo ( isset( $newarr['arf_bg_position_input_y'] ) && $newarr['arf_bg_position_input_y'] != '' ) ? esc_attr( $newarr['arf_bg_position_input_y'] ) : ''; ?>" class="arf_form_bg_position_input" onfocusout="arflite_set_form_bg_position(this, 'y', 'arf_fieldset_<?php echo esc_js( $id ); ?>')">
												</div>
											</div>
										</div>
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_form_padding arf_two_row_text"><?php echo  __( 'Form Padding', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_frm_padding_options arf_align_center arf_form_container">
												<div class="arf_form_padding_box_wrapper"><input type="text" name="arfmainfieldsetpadding_1" id="arfmainfieldsetpadding_1" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-top","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-top"}' value="<?php echo esc_attr( $newarr['arfmainfieldsetpadding_1'] ); ?>" class="arf_form_padding_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_padding" /><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Top', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_form_padding_box_wrapper"><input type="text" name="arfmainfieldsetpadding_2" id="arfmainfieldsetpadding_2" value="<?php echo esc_attr( $newarr['arfmainfieldsetpadding_2'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-right","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-right"}' class="arf_form_padding_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_padding" /><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Right', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_form_padding_box_wrapper"><input type="text" name="arfmainfieldsetpadding_3" id="arfmainfieldsetpadding_3" value="<?php echo esc_attr( $newarr['arfmainfieldsetpadding_3'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-bottom","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-bottom"}' class="arf_form_padding_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_padding" /><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Bottom', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_form_padding_box_wrapper"><input type="text" name="arfmainfieldsetpadding_4" id="arfmainfieldsetpadding_4" value="<?php echo esc_attr( $newarr['arfmainfieldsetpadding_4'] ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-left||.arflite_main_div_{arf_form_id} .arf_inner_wrapper_sortable.arfmainformfield.ui-sortable-helper~|~left","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~padding-left||.arflite_main_div_{arf_form_id} .arf_inner_wrapper_sortable.arfmainformfield.ui-sortable-helper~|~left"}' class="arf_form_padding_box" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_padding"data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_padding" /><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Left', 'arforms-form-builder' ) ; ?></span></div>
												<?php
										...
										<div class="arf_accordion_container_row arf_half_width arfdisablediv">
											<div class="arf_accordion_inner_title arf_form_padding arf_two_row_text"><?php echo  __( 'Section Padding', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_center arf_form_container arflite_section_field_padding">
												
												<div class="arf_section_padding_box_wrapper arf_restricted_control"><input type="text" name="arfsectionpaddingsetting_1" id="arfsectionpaddingsetting_1" onchange="arflite_change_field_padding('arfsectionpaddingsetting');" value="20" class="arf_section_padding_box" disabled="disabled" /><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Top', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_section_padding_box_wrapper arf_restricted_control"><input type="text" name="arfsectionpaddingsetting_2" id="arfsectionpaddingsetting_2" value="0" onchange="arflite_change_field_padding('arfsectionpaddingsetting');" class="arf_section_padding_box" disabled="disabled"/><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Right', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_section_padding_box_wrapper arf_restricted_control"><input type="text" name="arfsectionpaddingsetting_3" id="arfsectionpaddingsetting_3" value="20" onchange="arflite_change_field_padding('arfsectionpaddingsetting');" class="arf_section_padding_box" disabled="disabled"/><br /><span class="arf_px arf_font_size margin-left-10px" ><?php echo  __( 'Bottom', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_section_padding_box_wrapper arf_restricted_control"><input type="text" name="arfsectionpaddingsetting_4" id="arfsectionpaddingsetting_4" value="20" onchange="arflite_change_field_padding('arfsectionpaddingsetting');" class="arf_section_padding_box" disabled="disabled"/><br /><span class="arf_px arf_font_size margin-left-10px"><?php echo  __( 'Left', 'arforms-form-builder' ) ; ?></span></div>
												<?php
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Form Border', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Border Type', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_right arf_right">
												<div class="arf_toggle_button_group arf_two_button_group margin-right_5px">
													...
													<label class="arf_flat_border_btn arf_flat_border_btn_pdding arf_toggle_btn <?php echo ( $newarr['form_border_shadow'] == 'flat' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arffbs" class="visuallyhidden" value="flat" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow-none","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow-none"}'  id="arfmainformbordershadow2" data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_border_type" <?php checked( $newarr['form_border_shadow'], 'flat' ); ?> /><?php echo  __( 'Flat', 'arforms-form-builder' ) ; ?></label>
													<label class="arf_toggle_btn <?php echo ( $newarr['form_border_shadow'] == 'shadow' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arffbs" class="visuallyhidden" id="arfmainformbordershadow1" value="shadow" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow"}' <?php checked( $newarr['form_border_shadow'], 'shadow' ); ?> data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_border_type" /><?php echo  __( 'Shadow', 'arforms-form-builder' ) ; ?></label>
												</div>
											</div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo  __( 'Border Size', 'arforms-form-builder' ) ; ?></div>
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo  __( 'Border Radius', 'arforms-form-builder' ) ; ?></div>
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Window Opacity', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo  __( 'Window Opacity', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_center arf_accordion_container_mar">
												<div class="arf_slider_wrapper">
													...
													<div class="arf_slider_unit_data">
														<div class="arflite_float_left"><?php echo  __( '0', 'arforms-form-builder' ) ; ?></div>
														<div class="arflite_float_right"><?php echo  __( '1', 'arforms-form-builder' ) ; ?></div>
													</div>
							...
							<dl class="arf_accordion_tab_input_settings">
								<dd>
									<a href="javascript:void(0)" data-target="arf_accordion_tab_input_settings"><?php echo __( 'Input field Options', 'arforms-form-builder' ); ?></a>
									<div class="arf_accordion_container">
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo __( 'Label Options', 'arforms-form-builder' ); ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Label Position', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_right arf_width_50 arfhieght35 arf_right">
												...
												<div class="arf_toggle_button_group arf_three_button_group margin-right_5px">
													...
													?>
													" class="padding-7-10"><input type="radio" name="arfmps" class="visuallyhidden" onchange="arflitefrmSetPosClass('right');" <?php echo ( 'material' == $newarr['arfinputstyle'] ) ? ' disabled="disabled" ' : '' ?> value="right" <?php echo esc_attr($checked_right); ?> /><?php echo  __( 'Right', 'arforms-form-builder' ) ; ?></label>
													...
													?>
													"><input type="radio" name="arfmps" class="visuallyhidden" onchange="arflitefrmSetPosClass('left');" <?php echo ( 'material' == $newarr['arfinputstyle'] ) ? ' disabled="disabled" ' : ''; ?> value="left" <?php checked( esc_html( $newarr['position'] ), 'left' ); ?> /><?php echo  __( 'Left', 'arforms-form-builder' ) ; ?></label>
													<label class="arf_toggle_btn arf_label_position padding-7-10 arf_top_position <?php echo ( $checked_top != '' ) ? 'arf_success' : ''; ?>"><input type="radio" name="arfmps" class="visuallyhidden" onchange="arflitefrmSetPosClass('top');" value="top" <?php echo esc_attr($checked_top); ?> /><?php echo  __( 'Top', 'arforms-form-builder' ) ; ?></label>
												</div>
											</div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Label Align', 'arforms-form-builder' ) ; ?></div>
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo __( 'Hide Label', 'arforms-form-builder' ); ?></div>
											<div class="arf_accordion_content_container arf_align_right">
												<div class="arf_float_right margin-right_5px">
													<label class="arf_js_switch_label">
														<span class=""><?php echo  __( 'No', 'arforms-form-builder' ) ; ?>&nbsp;</span>
													</label>
													<span class="arf_js_switch_wrapper">
													...
													<label class="arf_js_switch_label">
														<span class="">&nbsp;<?php echo  __( 'Yes', 'arforms-form-builder' ) ; ?></span>
													</label>
												</div>
											</div>
										</div>
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Input Field Description Options', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Font Size', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50 arf_right">
												<div class="arf_dropdown_wrapper arflite_margin_right5" style="margin-right: 5px;width: 60px;">
													<?php
                                                        ...
                                                        echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfdfss', 'arfdescfontsizesetting', '', '', $newarr['arfdescfontsizesetting'], $font_size_attr, $font_size_opts );
                                                    ?>
												</div>
											</div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Text Alignment', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50 arf_right">
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Input Field Options', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Field Width', 'arforms-form-builder' ) ; ?></div>
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo __( 'Hide Required Indicator', 'arforms-form-builder' ); ?></div>
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text arf_width_50"><?php echo  __( 'Space Between Two Fields', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50 arf_right arfinputstyledrpdwn_container">
												<input type="text" name="arffms" id="arffieldmarginsetting" class="arf_small_width_txtbox arfcolor" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} #new_fields .arfmainformfield.edit_form_item~|~field-margin-bottom","material":".arflite_main_div_{arf_form_id} #new_fields .arfmainformfield.edit_form_item~|~field-margin-bottom"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_space_between_fields" value="<?php echo esc_attr( $newarr['arffieldmarginssetting'] ); ?>"  size="5" />
											</div>
										</div>

										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text">
												<?php echo  __( 'Placeholder Opacity', 'arforms-form-builder' ) ; ?>
											</div>
											<div class="arf_accordion_content_container arf_align_center arf_accordion_container_mar">
												<div class="arf_slider_wrapper">
										...
										<div class="arf_accordion_container_row arf_half_width arflitefont-setting-wrap">
											<div class="arf_accordion_container_inner_div">
												<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Font Settings', 'arforms-form-builder' ) ; ?></div>
											</div>
											...
											<div class="arf_accordion_container_inner_div">
												<div class="arf_custom_font" data-id="arf_input_font_settings">
													...
													<div class="arf_custom_font_label"><?php echo  __( 'Advanced font options', 'arforms-form-builder' ) ; ?></div>
												</div>
											</div>
										</div>
										...
										<div class="arf_accordion_container_row_separator"></div>
                                        <div class="arf_accordion_container_row arf_padding">
                                            <div class="arf_accordion_outer_title"><?php echo __('Tooltip position', 'arforms-form-builder'); ?></div>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width">
                                            ...
                                            <div class="arf_accordion_content_container">      
                                                <div class="arf_toggle_button_group arf_four_button_group">
                                                    ...
                                                    <label id="leftpos" class="arf_toggle_btn lefttoolpos <?php echo ($newarr['arftooltipposition'] == 'left') ? 'arf_success' : ''; ?>"><input type="radio" name="arflitetippos" class="visuallyhidden arftooltipposition" id='left_pos' data-id="arfestbc2" value="left" <?php checked($newarr['arftooltipposition'], 'left'); ?> /><?php echo __('Left', 'arforms-form-builder'); ?></label>
                                                    <label id="bottompos" class="arf_toggle_btn bottomtoolpos <?php echo ($newarr['arftooltipposition'] == 'bottom') ? 'arf_success' : ''; ?>"><input type="radio" name="arflitetippos" class="visuallyhidden arftooltipposition" id='bottom_pos'  data-id="arfestbc2" value="bottom" <?php checked($newarr['arftooltipposition'], 'bottom'); ?> /><?php echo __('Bottom', 'arforms-form-builder'); ?></label>
                                                    <label id="toppos" class="arf_toggle_btn toptoolpos <?php echo ($newarr['arftooltipposition'] == 'top' ) ? 'arf_success' : ''; ?>"><input type='radio' name='arflitetippos' class='visuallyhidden arftooltipposition' id='top_pos' value='top' <?php checked($newarr['arftooltipposition'], 'top'); ?> /><?php echo __('Top', 'arforms-form-builder'); ?></label>
                                                </div>
                                            </div>
                                        </div>
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Field inner spacing', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Vertical', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_center arf_accordion_container_mar">
												<div class="arf_slider_wrapper">
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Horizontal', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_center arf_accordion_container_mar">
												<div class="arf_slider_wrapper">
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Field Border Settings', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Border Size', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_center arf_accordion_container_mar">
												<div class="arf_slider_wrapper">
										...
										<div class="arf_accordion_container_row arf_half_width">
												...
												<div class="arf_dropdown_wrapper" style="margin-right: 5px;float:right;">
                                                    ...
                                                    if ($wp_format_date == 'F j, Y' || $wp_format_date == 'm/d/Y') {
                                                        ...
                                                        <div class="sltstandard1" style="float:left;">
                                                            ...
                                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arffdaf', 'frm_date_format', '', 'width:155px', $newarr['date_format'], $wp_format_date_attr, $wp_format_date_opts );
                                                    ...
                                                    ?>
                                                        ...
                                                        <div class="sltstandard1" style="float:left;">
                                                            ...
                                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arffdaf', 'frm_date_format', '', 'width:122px', $newarr['date_format'], $wp_format_date_attr, $wp_format_date_opts );
                                                    ...
                                                    <?php }else if($wp_format_date == 'd.F.y' || $wp_format_date == 'd.m.Y' || $wp_format_date == 'Y.m.d' || $wp_format_date == 'd. F Y' || $wp_format_date == 'm.d.Y') { ?>
										 ...
										 <div class="arf_accordion_container_row_separator"></div>
                                        <div class="arf_accordion_container_row arf_padding">
                                            <div class="arf_accordion_outer_title"><?php echo addslashes(esc_html__('PageBreak Timer Settings', 'arforms-form-builder')); ?><span class="arflite_pro_version_notice">(Premium)</span></div>
                                        </div>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width">
                                            <div class="arf_accordion_inner_title"><?php echo addslashes(esc_html__('Add Timer', 'arforms-form-builder')); ?>
                                            </div>
                                            <div class="arf_float_right arfmarginright4">
                                                <label class="arf_js_switch_label">
                                                    <span><?php echo addslashes(esc_html__('No', 'arforms-form-builder')); ?>&nbsp;</span>
                                                </label>
                                                ...
                                                <label class="arf_js_switch_label">
                                                    <span>&nbsp;<?php echo addslashes(esc_html__('Yes', 'arforms-form-builder')); ?></span>
                                                </label>
                                            </div>
                                        </div>
                                            ...
                                            <div class="arf_accordion_container_row arf_half_width" id="arfpagebreak_settimeron" <?php if(isset($newarr['arfsettimer']) && $newarr['arfsettimer'] == 0){ ?> style="display: none;" <?php } ?> >
                                                <div class="arf_accordion_inner_title"><?php echo addslashes(esc_html__('Set Timer On', 'arforms-form-builder')); ?></div>
                                                <div class="arf_accordion_content_container arf_align_right arf_right">                                                    
                                                    <div class="arf_toggle_button_group arf_three_button_group">
                                                        ...
                                                        <label class="arf_toggle_btn <?php echo ($newarr['arfpagebreaksettimeron'] == 'individualstep') ? 'arf_success' : ''; ?>">
                                                            <input type="radio" name="arfpagebreaksettimeron" class="visuallyhidden arf_restricted_control " id="arfpagebreaksettimeron_individualstep" value="individualstep" <?Php checked($newarr['arfpagebreaksettimeron'], 'individualstep'); ?> /><?php echo addslashes(esc_html__('Individual Steps', 'arforms-form-builder')); ?>
                                                        </label>
                                                        <label class="arf_toggle_btn <?php echo ($newarr['arfpagebreaksettimeron'] == 'overallform') ? 'arf_success' : ''; ?>">
                                                            <input type="radio" name="arfpagebreaksettimeron" class="visuallyhidden arf_restricted_control " id="arfpagebreaksettimeron_overallform" value="overallform" <?Php checked($newarr['arfpagebreaksettimeron'], 'overallform'); ?> /><?php echo addslashes(esc_html__('Overall Steps', 'arforms-form-builder')); ?>
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width" id="showunits_breakfield" <?php if(isset($newarr['arfsettimer']) && $newarr['arfsettimer'] == 0){ ?> style="display: none;" <?php } ?> >
                                            <div class="arf_accordion_inner_title arf_two_row_text"><?php echo addslashes(esc_html__('Show Units', 'arforms-form-builder')); ?></div>
                                            <div class="arf_accordion_content_container arf_align_right arf_right">         
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width" id="settimerval" <?php if(isset($newarr['arfsettimer']) && $newarr['arfsettimer'] == 0){ ?> style="display: none; <?php } ?>"> 
                                            <div class="arf_accordion_inner_title arf_form_padding arf_two_row_text"><?php echo addslashes(esc_html__('Set Timer', 'arforms-form-builder')); ?></div>
                                            <div class="arf_accordion_content_container arf_align_right arf_right">
                                                ...
                                                <span class="arf_px arf_font_size" style="margin:0 14px 0 17px;"><?php echo addslashes(esc_html__('Hrs', 'arforms-form-builder')); ?></span></div>
                                                ...
                                                <span class="arf_px arf_font_size" style="margin:0 14px 0 17px;"><?php echo addslashes(esc_html__('Min', 'arforms-form-builder')); ?></span></div>
                                                ...
                                                <span class="arf_px arf_font_size" style="margin:0 14px 0 17px;"><?php echo addslashes(esc_html__('Sec', 'arforms-form-builder')); ?></span></div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width arf_restricted_control" id="timerstyle" <?php if(isset($newarr['arfsettimer']) && $newarr['arfsettimer'] == 0){ ?> style="display: none; <?php } ?>"> 
                                            <div class="arf_accordion_inner_title"><?php echo addslashes(esc_html__('Timer Style', 'arforms-form-builder')); ?></div>
                                            <div class="arf_accordion_content_container">
                                                ...
                                                <div class="arf_dropdown_wrapper" style="width: 100%;">
                                                    <?php
                                                        ...
                                                        echo $arflitemaincontroller->arflite_selectpicker_dom( 'arf_page_break_style', 'arf_page_break_style', 'arf_selectbox_option arf_restricted_control arf_disabled', 'width:122px', $pagebreak_selected_style, array() , $pagebreak_style_opt );
                                                    ?>
                                                </div>
                                                </div>
                                            </div>
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Checkbox & Radio Style', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Style', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container">
												<div class="arf_dropdown_wrapper" style="margin-right: 5px;">
                                                    <?php
                                                        ...
                                                        echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfcksn', 'frm_check_radio_style', '', 'width:122px', $newarr['arfcheckradiostyle'], $arf_checkbox_attrs, $arf_checkbox_options, false, $options_class_arr );
										...
										<div class="arf_accordion_container_row arf_half_width" id="check_radio_main_icon" style="<?php echo ( $newarr['arfcheckradiostyle'] == 'custom' ) ? 'display:block;margin-bottom: 20px;height: auto;' : 'display:none;margin-bottom: 20px;height: auto;'; ?>">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Icon', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50 " style="margin-right: -1px;">
												<div class="arf_field_check_radio_wrapper" id="arf_field_check_radio_wrapper arf_right arf_accordion_container_mar">
													<div class="custom_checkbox_wrapper">
														...
														<div class="howto"> <?php echo  __( 'CheckBoxes', 'arforms-form-builder' ) ; ?> </div>
													</div>
													<br>
													<br>
													<div class="custom_checkbox_wrapper">
														...
														<div class="howto"> <?php echo  __( 'Radio Buttons', 'arforms-form-builder' ) ; ?> </div>
													</div>
												</div>
											</div>
										</div>
									...
									<?php do_action('arflite_add_form_additional_input_settings', $id, $values); ?> 
									</div>
								</dd>
							</dl>
                            ...
                            <dl class="arf_accordion_tab_field_animation_settings arf_field_animation_main_container">
                                <dd>
                                    <a href="javascript:void(0)" data-target="arf_accordion_tab_field_animation_settings"><?php echo __('Field Animation Options', 'arforms-form-builder'); ?><span class="arflite_pro_version_notice">(Premium)</span></a>
                                    <div class="arf_accordion_container">
                                    	<div class="arf_accordion_container_row arf_padding">
                                            <div class="arf_accordion_outer_title"><?php echo __('On Load Animation', 'arforms-form-builder'); ?></div>
                                        </div>
                                        <div class="arf_accordion_container_row arf_half_width">
                                        	<div class="arf_accordion_inner_title"><?php echo addslashes(esc_html__('Animation', 'arforms-form-builder')); ?></div>
                                            <div class="arf_accordion_content_container">
                                            	<?php
                                            	...
                                            	$no_animation = array(
                                                    ...
                                                    echo $arflitemaincontroller->arflite_selectpicker_dom( 'arf_editor_form_width_unit', 'arf_editor_form_width_unit', 'arf_restricted_control arf_disabled', 'width:170px;', '',$ar_disable_animation,$no_animation);
                                                ?>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width" style="height: auto;">
                                            ...
                                            <div class="arf_accordion_container_inner_div">
                                                <div class="arf_accordion_inner_title arf_width_50"><?php echo addslashes(esc_html__('Preview','arforms-form-builder')); ?></div>
                                            </div>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width">
                                            ...
                                            <div class="arf_accordion_content_container arf_align_right">
                                                <div class="arf_float_right" style="margin-right:5px;">
                                                    <label class="arf_js_switch_label">
                                                        <span class=""><?php echo addslashes(esc_html__('No', 'arforms-form-builder')); ?>&nbsp;</span>
                                                    </label>
                                                    ...
                                                    <label class="arf_js_switch_label">
                                                        <span class="">&nbsp;<?php echo addslashes(esc_html__('Yes', 'arforms-form-builder')); ?></span>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="arf_accordion_container_row arf_half_width">
                                            ...
                                            <div class="arf_accordion_content_container">
                                            	...
                                            	$animation = array(
                                                    ...
                                                    echo $arflitemaincontroller->arflite_selectpicker_dom( 'arf_editor_form_width_unit', 'arf_editor_form_width_unit', 'arf_restricted_control arf_disabled', 'width:170px;', '','',$animation);
                                                ?>
                                            </div>
                                        </div>
							...
							<dl class="arf_accordion_tab_submit_settings">
								<dd>
									<a href="javascript:void(0)" data-target="arf_accordion_tab_submit_settings"><?php echo  __( 'Submit Button Options', 'arforms-form-builder' ) ; ?></a>
									<div class="arf_accordion_container">
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'General Options', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text"><?php echo  __( 'Button Alignment', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_right">
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text arf_width_50" ><?php echo  __( 'Button Height (optional)', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50">
												<span class="arfpxspan">px</span>
												<input type="text" name="arfsbhs" id="arfsubmitbuttonheightsetting" class="arf_small_width_txtbox arfcolor" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_submit_btn~|~height","material":".arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_submit_btn~|~height"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_submit_button_height" value="<?php echo esc_attr( $newarr['arfsubmitbuttonheightsetting'] ); ?>"  size="5" />
											</div>
										</div>
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Button Text', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container">
												<?php
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_width_50"><?php echo  __( 'Button Style', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_width_50 arf_right">
												<?php
                                                    ...
                                                    echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfsubmitbuttonstyle', 'arfsubmitbuttonstyle', '', '', $newarr['arfsubmitbuttonstyle'], $submit_button_style_attr, $submit_button_style_opts );
                                                ?>
											</div>
										</div>
										<div class="arf_accordion_container_row arf_half_width" style="margin-bottom: 30px;">
											<div class="arf_accordion_inner_title arf_form_padding" id="arf_sub_btn_margin"><?php echo  __( 'Margin', 'arforms-form-builder' ); ?></div>
											<div class="arf_accordion_content_container arf_form_container ">
												<div class="arf_submit_margin_box_wrapper"><input type="text" name="arfsubmitbuttonmarginsetting_1" id="arfsubmitbuttonmarginsetting_1" onchange="arflite_change_field_padding('arfsubmitbuttonmarginsetting');" value="<?php echo esc_attr( $newarr['arfsubmitbuttonmarginsetting_1'] ); ?>" class="arf_submit_margin_box" /><br /><span class="arf_px arf_font_size" ><?php echo  __( 'Top', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_submit_margin_box_wrapper"><input type="text" name="arfsubmitbuttonmarginsetting_2" id="arfsubmitbuttonmarginsetting_2" value="<?php echo esc_attr( $newarr['arfsubmitbuttonmarginsetting_2'] ); ?>" onchange="arflite_change_field_padding('arfsubmitbuttonmarginsetting');" class="arf_submit_margin_box" /><br /><span class="arf_px arf_font_size" ><?php echo  __( 'Right', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_submit_margin_box_wrapper"><input type="text" name="arfsubmitbuttonmarginsetting_3" id="arfsubmitbuttonmarginsetting_3" value="<?php echo esc_attr( $newarr['arfsubmitbuttonmarginsetting_3'] ); ?>" onchange="arflite_change_field_padding('arfsubmitbuttonmarginsetting');" class="arf_submit_margin_box" /><br /><span class="arf_px arf_font_size" style="    margin-left: 5px;"><?php echo  __( 'Bottom', 'arforms-form-builder' ) ; ?></span></div>
												<div class="arf_submit_margin_box_wrapper"><input type="text" name="arfsubmitbuttonmarginsetting_4" id="arfsubmitbuttonmarginsetting_4" value="<?php echo esc_attr( $newarr['arfsubmitbuttonmarginsetting_4'] ); ?>" onchange="arflite_change_field_padding('arfsubmitbuttonmarginsetting');" class="arf_submit_margin_box" /><br /><span class="arf_px arf_font_size" style="margin-left: 10px;"><?php echo  __( 'Left', 'arforms-form-builder' ) ; ?></span></div>
											</div>
											<?php
										...
										<div class="arf_accordion_container_row arf_half_width">
											<div class="arf_accordion_inner_title arf_two_row_text "><?php echo  __( 'Background Hover Image', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_right arf_right">
										...
										<div class="arf_accordion_container_row_separator"></div>
										<div class="arf_accordion_container_row arf_padding">
											<div class="arf_accordion_outer_title"><?php echo  __( 'Border Options', 'arforms-form-builder' ) ; ?></div>
										</div>
										<div class="arf_accordion_container_row arf_half_width arf_accordion_container_mar">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Size', 'arforms-form-builder' ) ; ?></div>
										...
										<div class="arf_accordion_container_row arf_half_width arf_accordion_container_mar">
											<div class="arf_accordion_inner_title"><?php echo  __( 'Radius', 'arforms-form-builder' ) ; ?></div>
											<div class="arf_accordion_content_container arf_align_center">
												<div class="arf_slider_wrapper">
								...
								<div class="arf_accordion_container_row_separator"></div>
									<div class="arf_accordion_container_row arf_padding">
										<div class="arf_accordion_outer_title"><?php echo  __( 'Shadow Options', 'arforms-form-builder' ) ; ?></div>
									</div>
									<div class="arf_accordion_container_row arf_half_width arf_accordion_container_mar">
										<div class="arf_accordion_inner_title"><?php echo  __( 'X-offset', 'arforms-form-builder' ) ; ?></div>
										<div class="arf_accordion_content_container arf_align_center">
											<div class="arf_slider_wrapper">
									...
									<div class="arf_accordion_container_row arf_half_width arf_accordion_container_mar">
										<div class="arf_accordion_inner_title"><?php echo  __( 'Y-offset', 'arforms-form-builder' ) ; ?></div>
										<div class="arf_accordion_content_container arf_align_center">
											<div class="arf_slider_wrapper">
										...
										<div class="arf_accordion_container_row arf_half_width arf_accordion_container_mar">
										<div class="arf_accordion_inner_title"><?php echo  __( 'Blur', 'arforms-form-builder' ) ; ?></div>
										<div class="arf_accordion_content_container arf_align_center">
											<div class="arf_slider_wrapper">
									...
									<div class="arf_accordion_container_row arf_half_width arf_accordion_container_mar">
										<div class="arf_accordion_inner_title"><?php echo  __( 'Spread', 'arforms-form-builder' ) ; ?></div>
										<div class="arf_accordion_content_container arf_align_center">
											<div class="arf_slider_wrapper">
				...
				<div class="arf_form_style_tab_container" id="arf_form_custom_css">
					<div class="arf_form_custom_css_tab">
						...
						<div class="arf_custom_css_cloud_wrapper">
							<span><?php echo  __( 'Add CSS Elements', 'arforms-form-builder' ) ; ?></span>
							<i class="fas fa-caret-down fa-lg"></i>
				...
				<div class="arf_custom_color_popup">
					...
					?>
					<div class="arf_custom_color_popup_header"><?php echo  __( 'Custom Color', 'arforms-form-builder' ) ; ?></div>
					<div class="arf_custom_color_popup_container">
						<div class="arf_custom_color_popup_table">
							<div class="arf_custom_color_popup_table_row">
								<div class="arf_custom_color_popup_left_item" id="form_level_colors"><span><?php echo  __( 'Form', 'arforms-form-builder' ) ; ?></span></div>
								<div class="arf_custom_color_popup_right_item_wrapper">
									<div class="arf_custom_color_popup_right_item">
										...
										<input type="hidden" name="arffbcs" id="arfformbgcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $bgColor ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~background-color","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~background-color"}'  data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_background_color" />
										<?php echo  __( 'Background', 'arforms-form-builder' ) ; ?>
									</div>
									...
									<div class="arf_custom_color_popup_right_item">
										...
										<input type="hidden" name="arffboss" id="arfformbordershadowsetting" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow","property":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow","material":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow","property":".arflite_main_div_{arf_form_id} .arf_fieldset~|~box-shadow"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_border_type" class="hex txtxbox_widget " value="<?php echo str_replace( '##', '#', $formShadowColor ); ?>" /> <?php echo  __( 'Shadow', 'arforms-form-builder' ) ; ?>
									</div>
									...
									<div class="arf_custom_color_popup_right_item arfsectionbgwrap arfdisablediv">
										...
										<label for="arf_section_inherit_bg" class="arfsection-bgcolor-lbl"><?php echo  __( 'Section Background', 'arforms-form-builder' ) ; ?></label>
									</div>
							...
							<div class="arf_custom_color_popup_table_row">
								<div class="arf_custom_color_popup_left_item" id="input_colors"><span><?php echo  __( 'Main Input Colors', 'arforms-form-builder' ) ; ?></span></div>
								<div class="arf_custom_color_popup_right_item_wrapper">
									...
									<div class="arf_custom_color_popup_right_item">
										...
										<input type="hidden" name="arfmbsc" data-arfstyle="true" data-arfstyledata='<?php echo esc_attr(json_encode( $skinJson->css_main_classes )); ?>' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_main_style" value="<?php echo esc_attr($inputBaseColor); ?>" id="arfmainbasecolor" class="txtxbox_widget hex" style="width:100%;" />
										<?php echo  __( 'Base/Active Color', 'arforms-form-builder' ) ; ?>
									</div>
									...
									<div class="arf_custom_color_popup_right_item <?php echo ( $newarr['arfinputstyle'] == 'material' ) ? 'arfdisablediv' : ''; ?>">
										...
										<input type="hidden" name="arfbecs" id="arfbgerrorcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $inputErrorBg ); ?>" />
										<?php echo  __( 'Error State Background', 'arforms-form-builder' ) ; ?>
									</div>
									...
									<div class="arf_custom_color_popup_right_item <?php echo ( $newarr['arfinputstyle'] == 'material' ) ? 'arfdisablediv' : ''; ?>">
										...
										<input type="hidden" name="pfsfscol" id="prefix_suffix_icon_color" class="txtxbox_widget  hex" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .controls .arf_editor_prefix_suffix_wrapper .arf_editor_prefix_icon~|~color||.arflite_main_div_{arf_form_id} .controls .arf_editor_prefix_suffix_wrapper .arf_editor_suffix_icon~|~color","material":".arflite_main_div_{arf_form_id} .controls .arf_editor_prefix_suffix_wrapper .arf_editor_prefix_icon~|~color||.arflite_main_div_{arf_form_id} .controls .arf_editor_prefix_suffix_wrapper .arf_editor_suffix_icon~|~color"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_icon_color" value="<?php echo str_replace( '##', '#', $iconColor ); ?>" />
										<?php echo  __( 'Icon Color', 'arforms-form-builder' ) ; ?>
									</div>


									<input type="hidden" name="cbscol" id="checked_checkbox_icon_color" class="txtxbox_widget hex" value="<?php echo isset( $newarr['checked_checkbox_icon_color'] ) ? str_replace( '##', '#', $newarr['checked_checkbox_icon_color'] ) : ''; ?>" />
									<input type="hidden" name="rbscol" id="checked_radio_icon_color" class="txtxbox_widget hex" value="<?php echo isset( $newarr['checked_radio_icon_color'] ) ? ( str_replace( '##', '#', $newarr['checked_radio_icon_color'] ) ) : ''; ?>" />
									<div class="arf_popup_clear"></div>


									<span class="arf_custom_color_popup_subtitle"><?php echo  __( 'Like Button', 'arforms-form-builder' ) ; ?></span>
									<div class="arf_custom_color_popup_right_item arfdisablediv arf_restricted_control">
										...
										<input type="hidden" name="albclr" id="editor_like_button_color" class="txtxbox_widget" value="#4786ff" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_like_btn.active~|~background","material":".arflite_main_div_{arf_form_id} .arf_like_btn.active~|~background"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_like_button_color" /><?php echo  __( 'Like Button Color', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_custom_color_popup_right_item arfdisablediv arf_restricted_control">
									...
									<div class="arf_custom_color_popup_right_item arfdisablediv arf_restricted_control">
										...
										<input type="hidden" name="asldrsl" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .slider.slider-horizontal .slider-selection~|~background","material":".arflite_main_div_{arf_form_id} .slider.slider-horizontal .slider-selection~|~background"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_slider_selection_color" id="editor_slider_left_side" class="txtxbox_widget " value="<?php echo str_replace( '##', '#', $sliderLeftColor ); ?>" /><?php echo  __( 'Slider selected', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_custom_color_popup_right_item arfdisablediv arf_restricted_control">
										<div class="arf_custom_color_popup_picker jscolor arfdisablediv arf_restricted_control" data-fid="editor_slider_right_side" style="background:<?php echo str_replace( '##', '#', $sliderRightColor ); ?>" data-skin="input.slider_track_color" data-jscolor='{"hash":true,"valueElement":"editor_slider_right_side","onfinechange":"arflite_update_color(this,\"editor_slider_right_side\")"}'></div>
										<input type="hidden" name="asltrcl" id="editor_slider_right_side" class="txtxbox_widget " value="<?php echo str_replace( '##', '#', $sliderRightColor ); ?>" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .slider.slider-horizontal .arf-slider-track~|~background","material":".arflite_main_div_{arf_form_id} .slider.slider-horizontal .arf-slider-track~|~background"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_slider_selection_color" /><?php echo  __( 'Slider Track', 'arforms-form-builder' ) ; ?>
									</div>

									<div class='arf_popup_clear'></div>

									<span class="arf_custom_color_popup_subtitle"><?php echo  __( 'Star Rating color', 'arforms-form-builder' ) ; ?></span>
									<div class="arf_custom_color_popup_right_item arfdisablediv arf_restricted_control">
										...
										<input type="hidden" name="asclcl" id="editor_rating_color" class="txtxbox_widget " value="#FCBB1D" /><?php echo  __( 'Star Rating Color', 'arforms-form-builder' ) ; ?>
									</div>


									<div class='arf_popup_clear'></div>

									<span class="arf_custom_color_popup_subtitle"><?php echo  __( 'Field Tooltip', 'arforms-form-builder' ) ; ?></span>
									...
									<div class="arf_custom_color_popup_right_item">
										...
										<input type="hidden" name="arf_tooltip_font_color" id="arf_tooltip_font_color" class="txtxbox_widget  hex" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arf_fieldset .arf_tooltip_main~|~color","material":".arflite_main_div_{arf_form_id} .arf_fieldset .arf_tooltip_main~|~color"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_tooltip_txt_color" value="<?php echo str_replace( '##', '#', $newarr['arf_tooltip_font_color'] ); ?>" onchange="arflitetooltipinitialization();"/>
										<?php echo  __( 'Text Color', 'arforms-form-builder' ) ; ?>
									</div>
									...
									<span class="arf_custom_color_popup_subtitle"><?php echo  __('Matrix Field Background Color', 'arforms-form-builder') ; ?></span>
									  <div class="arf_custom_color_popup_right_item" id="arf_matrix_odd_bgcolor_wrapper">
                                        ...
                                        <?php echo __('Odd Row', 'arforms-form-builder'); ?>
                                    </div>
                                    <div class="arf_custom_color_popup_right_item" id="arf_matrix_even_bgcolor_wrapper">
                                        ...
                                        <?php echo __('Even Row', 'arforms-form-builder'); ?>
                                    </div>
                                    ...
                                    <div class="arf_custom_color_popup_right_item" style="width: auto;margin-top: 20px;<?php echo (is_rtl()) ? 'margin-right:20px;margin-left:0px;' : 'margin-left:20px;margin-right:0;'; ?>">
                                        ...
                                        <label for="arf_matrix_inherit_bg" style="<?php echo (is_rtl()) ? 'float: right;text-align: right;margin-right: -3px;position: relative;' : 'float: left;text-align: left;margin-left: -3px;'; ?>margin-top: 3px;"><?php echo __('Set Transparent Background','arforms-form-builder'); ?></label>
                                    </div>
									<span class="arf_custom_color_popup_subtitle"><?php echo  __( 'Other color', 'arforms-form-builder' ) ; ?></span>
							...
							<div class="arf_custom_color_popup_table_row">
								<div class="arf_custom_color_popup_left_item" id="submit_button_colors"><span><?php echo  __( 'Submit Button Colors', 'arforms-form-builder' ) ; ?></span></div>
								<div class="arf_custom_color_popup_right_item_wrapper">
									<div class="arf_custom_color_popup_right_item">
										...
										<input type="hidden" name="arfsbtcs" id="arfsubmitbuttontextcolorsetting" class="txtxbox_widget  hex" data-arfstyle="true" data-arfstyledata='{"standard":".arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_submit_btn~|~color||.arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_greensave_button_wrapper .arf_submit_btn:hover.arf_submit_btn_border~|~color||.arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_greensave_button_wrapper .arf_submit_btn.arf_submit_btn_reverse_border~|~color||.arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_greensave_button_wrapper .arf_submit_btn.arf_submit_btn_flat~|~color","material":".arflite_main_div_{arf_form_id} .arfsubmitbutton .arf_submit_btn~|~color||.arflite_main_div_{arf_form_id} .arf_materialize_form .arfsubmitbutton .arf_greensave_button_wrapper .arf_submit_btn:hover.arf_submit_btn_border~|~color||.arflite_main_div_{arf_form_id} .arf_materialize_form .arfsubmitbutton .arf_greensave_button_wrapper .arf_submit_btn.arf_submit_btn_reverse_border~|~color||.arflite_main_div_{arf_form_id} .arf_materialize_form .arfsubmitbutton .arf_greensave_button_wrapper .arf_submit_btn.arf_submit_btn_flat~|~color"}' data-arfstyleappend="true" data-arfstyleappendid="arf_{arf_form_id}_form_submit_button_color" value="<?php echo str_replace( '##', '#', $submitTxtColor ); ?>" />
										<?php echo  __( 'Text Color', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_custom_color_popup_right_item">
							...
							<div class="arf_custom_color_popup_table_row">
								<div class="arf_custom_color_popup_left_item" id="success_message_colors"><span><?php echo  __( 'Success message Colors', 'arforms-form-builder' ) ; ?></span></div>
								<div class="arf_custom_color_popup_right_item_wrapper">
									...
									<div class="arf_custom_color_popup_right_item">
										<div class="arf_custom_color_popup_picker jscolor" data-fid="arfmainsucessbgcolorsetting" style="background:<?php echo str_replace( '##', '#', $successBgColor ); ?>;" data-skin="success_msg.background" data-default-color="<?php echo str_replace( '##', '#', $successBgColor ); ?>" data-checkskin="true" data-jscolor='{"hash":true,"valueElement":"arfmainsucessbgcolorsetting","onfinechange":"arflite_update_color(this,\"arfmainsucessbgcolorsetting\")"}'></div>
										<input name="arfmsbcs" id="arfmainsucessbgcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $successBgColor ); ?>" type="hidden" />
										<?php echo  __( 'Background', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_custom_color_popup_right_item">
										<div class="arf_custom_color_popup_picker jscolor" data-fid="arfmainsucessbordercolorsetting" style="background:<?php echo str_replace( '##', '#', $successBrdColor ); ?>;" data-skin="success_msg.border" data-default-color="<?php echo str_replace( '##', '#', $successBrdColor ); ?>" data-checkskin="true" data-jscolor='{"hash":true,"valueElement":"arfmainsucessbordercolorsetting","onfinechange":"arflite_update_color(this,\"arfmainsucessbordercolorsetting\")"}'></div>
										<input type="hidden" name="arfmsbocs" id="arfmainsucessbordercolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $successBrdColor ); ?>" />
										<?php echo  __( 'Border', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_custom_color_popup_right_item">
										<div class="arf_custom_color_popup_picker jscolor" data-fid="arfmainsucesstextcolorsetting" style="background:<?php echo str_replace( '##', '#', $successTxtColor ); ?>;" data-skin="success_msg.text" data-default-color="<?php echo str_replace( '##', '#', $successTxtColor ); ?>" data-checkskin="true" data-jscolor='{"hash":true,"valueElement":"arfmainsucesstextcolorsetting","onfinechange":"arflite_update_color(this,\"arfmainsucesstextcolorsetting\")"}'></div>
										<input name="arfmstcs" id="arfmainsucesstextcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $successTxtColor ); ?>" type="hidden" />
										<?php echo  __( 'Text', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_popup_clear"></div>
								</div>
							</div>
							<div class="arf_custom_color_popup_table_row">
								<div class="arf_custom_color_popup_left_item" id="error_message_colors"><span><?php echo  __( 'Error Message Colors', 'arforms-form-builder' ) ; ?></span></div>
								<div class="arf_custom_color_popup_right_item_wrapper">
									<div class="arf_custom_color_popup_right_item">
										<div class="arf_custom_color_popup_picker jscolor" data-fid="arfformerrorbgcolorsetting" style="background:<?php echo str_replace( '##', '#', $errorBgColor ); ?>" data-skin="error_msg.background" data-default-color="<?php echo str_replace( '##', '#', $errorBgColor ); ?>" data-checkskin="true" data-jscolor='{"hash":true,"valueElement":"arfformerrorbgcolorsetting","onfinechange":"arflite_update_color(this,\"arfformerrorbgcolorsetting\")"}'></div>
										<input name="arffebgc" id="arfformerrorbgcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $errorBgColor ); ?>" type="hidden" />
										<?php echo  __( 'Background', 'arforms-form-builder' ) ; ?>
									</div>
									...
									<div class="arf_custom_color_popup_right_item">
										...
										<input name="arffebrdc" id="arfformerrorbordercolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $errorBrdColor ); ?>" type="hidden" />
										<?php echo  __( 'Border', 'arforms-form-builder' ) ; ?>
									</div>

									<div class="arf_custom_color_popup_right_item">
										<div class="arf_custom_color_popup_picker jscolor" data-fid="arfformerrortextcolorsetting" style="background:<?php echo str_replace( '##', '#', $errorTxtColor ); ?>" data-skin="error_msg.text" data-default-color="<?php echo str_replace( '##', '#', $errorTxtColor ); ?>" data-checkskin="true" data-jscolor='{"hash":true,"valueElement":"arfformerrortextcolorsetting","onfinechange":"arflite_update_color(this,\"arfformerrortextcolorsetting\")"}'></div>
										<input name="arffetxtc" id="arfformerrortextcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $errorTxtColor ); ?>" type="hidden" />
										<?php echo  __( 'Text', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_popup_clear"></div>
								</div>
							...
							<div class="arf_custom_color_popup_table_row">
								<div class="arf_custom_color_popup_left_item" id="validation_message_colors"><span><?php echo  __( 'Validation Message Colors', 'arforms-form-builder' ) ; ?></span></div>
								<div class="arf_custom_color_popup_right_item_wrapper">
									<div class="arf_custom_color_popup_right_item" id="arf_validation_background_color">
										...
										<input name="arfmvbcs" id="arfmainvalidationbgcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $validationBgColor ); ?>" type="hidden"  />
										<span><?php echo ( $newarr['arferrorstyle'] == 'normal' ) ?  __( 'Color', 'arforms-form-builder' )  :  __( 'Background', 'arforms-form-builder' ) ; ?></span>
									</div>
									<div class="arf_custom_color_popup_right_item" id="arf_validation_text_color" style="<?php echo ( $newarr['arferrorstyle'] == 'normal' ) ? 'display:none;' : 'display:block;'; ?>">
										...
										<input name="arfmvtcs" id="arfmainvalidationtextcolorsetting" class="txtxbox_widget  hex" value="<?php echo str_replace( '##', '#', $validationTxtColor ); ?>" type="hidden"  />
										<?php echo  __( 'Text', 'arforms-form-builder' ) ; ?>
									</div>
									<div class="arf_popup_clear"></div>
								</div>
					...
					<div class="arf_custom_color_popup_footer">
						<div class="arf_custom_color_button_position">
							<div class="arf_custom_color_button" id="arf_custom_color_save_btn"><div class="arf_imageloader arf_form_style_custom_color_loader" id="arf_custom_color_loader"></div><?php echo  __( 'Apply', 'arforms-form-builder' ) ; ?></div>
							<div class="arf_custom_color_button arf_custom_color_cancel" id="arf_custom_color_cancel_btn"><?php echo  __( 'Cancel', 'arforms-form-builder' ) ; ?></div>
						</div>
					</div>
				</div>
				
				<div class="arf_custom_font_popup">
					<div class="arf_custom_color_popup_header"><?php echo  __( 'Custom Font Options', 'arforms-form-builder' ) ; ?></div>
					<div class="arf_custom_font_popup_container">
						<div class="arf_accordion_container_row arf_margin">
							<div class="arf_accordion_outer_title"><?php echo  __( 'Form Title Font Settings', 'arforms-form-builder' ) ; ?></div>
						</div>
						<?php
						...
						<div class="arf_font_setting_class">
							<div class="arf_font_style_popup_row">
								<div class="arf_font_style_popup_left"><?php echo  __( 'Family', 'arforms-form-builder' ) ; ?></div>
								<div class="arf_font_style_popup_right">
									<div class="arf_dropdown_wrapper">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arftff', 'arftitlefontsetting', '','', $newarr['arftitlefontfamily'], $fontsattr, $fontsarr, true, array(), false, array(), false, array(), true );
							...
							<div class="arf_font_style_popup_row">
								<div class="arf_font_style_popup_left"><?php echo  __( 'Size', 'arforms-form-builder' ) ; ?></div>
								<div class="arf_font_style_popup_right arfwidth63">
									<div class="arf_dropdown_wrapper arfmarginleft">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfftfss', 'arfformtitlefontsizesetting', 'arflite_font_size_dropdown', '', $newarr['form_title_font_size'], $font_size_attr, $font_size_opts );
                                        ?>
                                    </div>
							...
							<div class="arf_font_style_popup_row">
								<div class="arf_font_style_popup_left"><?php echo  __( 'Style', 'arforms-form-builder' ) ; ?></div>
								<div class="arf_font_style_popup_right">
						...
						<div class="arf_accordion_container_row_separator"></div>
						<div class="arf_accordion_container_row arf_margin">
							<div class="arf_accordion_outer_title"><?php echo  __( 'Label Font Settings', 'arforms-form-builder' ) ; ?></div>
						</div>
						<?php
						$label_font_weight = '';
								...
								<div class="arf_font_style_popup_right">
									<div class="arf_dropdown_wrapper">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfmfs', 'arfmainfontsetting', '','', $newarr['font'], $fontsattr, $fontsarr, true, array(), false, array(), false, array(), true );
                                        ?>
                                    </div>
								</div>
							</div>
								...
								<div class="arf_font_style_popup_right arfwidth63">
									<div class="arf_dropdown_wrapper arfmarginleft">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arffss', 'arffontsizesetting', 'arflite_font_size_dropdown', '', $newarr['font_size'], $font_size_attr, $font_size_opts );
                                        ?>
                                    </div>
						...
						<div class="arf_accordion_container_row arf_margin" id="arf_input_font_settings_container">
							<div class="arf_accordion_outer_title"><?php echo  __( 'Input Font Settings', 'arforms-form-builder' ) ; ?></div>
						</div>
						<?php
								...
								<div class="arf_font_style_popup_right">
									<div class="arf_dropdown_wrapper">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfcbfs', 'arfcheckboxfontsetting', '','', $newarr['check_font'], $fontsattr, $fontsarr, true, array(), false, array(), false, array(), true );
								...
								<div class="arf_font_style_popup_right arfwidth63">
									<div class="arf_dropdown_wrapper arfmarginleft">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfffss', 'arffieldfontsizesetting', 'arflite_font_size_dropdown', '', $newarr['field_font_size'], $font_size_attr, $font_size_opts );
                                        ?>
                                    </div>
									<div class="arfwidthpx" style="<?php echo ( is_rtl() ) ? 'margin-right: 25px;margin-left: 0px;position:relative;' : 'margin-left: 25px;'; ?>">px</div>
								</div>
						...
						<div class="arf_accordion_container_row arf_margin arfdisablediv arf_restricted_control">
							<div class="arf_accordion_outer_title"><?php echo  __( 'Section Font Settings', 'arforms-form-builder' ) ; ?></div>
						</div>
								...
								<div class="arf_font_style_popup_right arfdisablediv">
									<div class="arf_dropdown_wrapper">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfsectiontitlefamily', 'arfsectiontitlefamily', 'arf_restricted_control','', $newarr['arfsectiontitlefamily'], array(), $fontsarr, true, array(), true, array(), false, array(), true );
                                        ?>
                                    </div>
								</div>
							</div>
								...
								<div class="arf_font_style_popup_right arfwidth63">
									<div class="arf_dropdown_wrapper arfmarginleft arfdisablediv">
										<?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfsectiontitlefontsizesetting', 'arfsectiontitlefontsizesetting', 'arf_restricted_control arflite_font_size_dropdown', '', $section_title_font_size, $font_size_attr, $font_size_opts, false, array(), true);
                                        ?>
									</div>
									<div class="arfwidthpx" style="<?php echo ( is_rtl() ) ? 'margin-right: 25px;margin-left: 0px;position:relative;' : 'margin-left: 25px;'; ?>">px</div>
								</div>
						...
						<div class="arf_accordion_container_row arf_margin" id="arf_submit_font_settings_container">
							<div class="arf_accordion_outer_title"><?php echo  __( 'Submit Font Settings', 'arforms-form-builder' ) ; ?></div>
						</div>
						<?php
								...
								<div class="arf_font_style_popup_right">
									<div class="arf_dropdown_wrapper">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfsff', 'arfsubmitfontfamily', '','', $newarr['arfsubmitfontfamily'], $fontsattr, $fontsarr, true, array(), false, array(), false, array(), true );
                                        ?>
                                    </div>
								</div>
							</div>
								...
								<div class="arf_font_style_popup_right arfwidth63">
									<div class="arf_dropdown_wrapper arfmarginleft">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfsbfss', 'arfsubmitbuttonfontsizesetting', 'arflite_font_size_dropdown', '', $newarr['arfsubmitbuttonfontsizesetting'], $font_size_attr, $font_size_opts );
                                        ?>
                                    </div>
									<div class="arfwidthpx" style="<?php echo ( is_rtl() ) ? 'margin-right: 25px;margin-left: 0px;position:relative;' : 'margin-left: 25px;'; ?>">px</div>
								</div>
						...
						<div class="arf_accordion_container_row_separator"></div>
						<div class="arf_accordion_container_row arf_margin">
							<div class="arf_accordion_outer_title"><?php echo  __( 'Validation Font Settings', 'arforms-form-builder' ) ; ?></div>
						</div>
								...
								<div class="arf_font_style_popup_right">
									<div class="arf_dropdown_wrapper">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfmefs', 'arfmainerrorfontsetting', '','', $newarr['error_font'], $fontsattr, $fontsarr, true, array(), false, array(), false, array(), true );
                                        ?>
                                    </div>
								</div>
							</div>
								...
								<div class="arf_font_style_popup_right arfwidth63">
									<div class="arf_dropdown_wrapper arfmarginleft">
                                        <?php
                                            ...
                                            echo $arflitemaincontroller->arflite_selectpicker_dom( 'arfmefss', 'arfmainerrorfontsizesetting', 'arflite_font_size_dropdown', '', $newarr['arffontsizesetting'], array(), $font_size_opts );
                                        ?>
                                    </div>
									<div class="arfwidthpx" style="<?php echo ( is_rtl() ) ? 'margin-right: 25px;margin-left: 0px;position:relative;' : 'margin-left: 25px;'; ?>">px</div>
								</div>
					...
					<div class="arf_custom_font_popup_footer">
						<div class="arf_custom_font_button_position">
							<div class="arf_custom_font_button arf_custom_font_save_close" id="arf_custom_font_save_btn"><?php echo  __( 'Apply', 'arforms-form-builder' ) ; ?></div>
							<div class="arf_custom_font_button arf_custom_font_cancel arf_custom_font_close" id="arf_custom_font_cancel_btn"><?php echo  __( 'Cancel', 'arforms-form-builder' ) ; ?></div>
						</div>
					</div>
				</div>
		...
		<div class="arf_modal_overlay">
			<div id="arf_mail_notification_model" class="arf_popup_container arf_popup_container_mail_notification_model">
				<div class="arf_popup_container_header"><?php echo  __( 'Email Notifications', 'arforms-form-builder' ) ; ?>
				...
				<div class="arf_popup_content_container arf_mail_notification_container">
					...
					<div class="arf_popup_checkbox_wrapper arfemail-notify-wrap">
						...
						<div class="arf_custom_checkbox_div">
							...
							<span><label id="arf_auto_responder" for="auto_responder" class="arffont16"><?php echo  __( 'Send an automatic response to users after form submission.', 'arforms-form-builder' ) ; ?></label></span>
						</div>
					...
					<div class="arf_auto_responder_content arfmarginl10" >
						<div class="arf_auto_responder_row">
							<div class="arf_auto_responder_column">
								<label class="arf_auto_responder_label arf_send_mail_to_label"><?php echo  __( 'Select field to send E-mail', 'arforms-form-builder' ) ; ?></label>
								<?php
								...
								$user_responder_email        = apply_filters( 'arflite_change_autoresponse_selected_email_value_in_outside', $user_responder_email, $id, $values );
								$selectbox_field_value_label = apply_filters( 'arflite_change_autoresponse_selected_email_label_in_outside', $selectbox_field_value_label, $id, $values );
                                ...
                                echo $arflitemaincontroller->arflite_selectpicker_dom( 'options[ar_email_to]', 'options_ar_user_email_to', $arf_autoresponder_cls, 'width:80%;margin-top: 7px;', $ar_email_to_val, $ar_email_to_attr, $selectbox_field_options, false, array(), $ar_email_to_opt_disable, array(), false, array(), true, 'arf_email_field_dropdown' );

								?>
								<div class="arf_popup_tooltip_main arfemail-tooltip"><img src="<?php echo ARFLITEIMAGESURL; ?>/tooltips-icon.png" alt="?" class="arfhelptip" title="<?php echo  __( 'Please map desired email field from the list of fields used in your form. And system will send response email to this address.', 'arforms-form-builder' ) ; ?>"/></div>
							</div>
							<div class="arf_auto_responder_column">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Subject E-mail', 'arforms-form-builder' ) ; ?></label>
								<?php
								...
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_subject')" id="add_field_email_subject_but" <?php echo ( isset( $values['auto_responder'] ) && $values['auto_responder'] < 1 ) ? ' disabled="disabled" ' : ''; ?>><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
									...
									<div class="arf_add_fieldmodal" id="add_field_subject">
										<div class="arf_modal_header">
											<div class="arf_add_field_title">
												<?php echo  __( 'Fields', 'arforms-form-builder' ) ; ?>
						...
						<div class="arf_auto_responder_row arfliteautoresor">
							<div class="arf_or_option"><?php echo  __( 'Or', 'arforms-form-builder' ) ; ?></div>
						</div>
						<div class="arf_auto_responder_row" style="margin-bottom:20px;">
							<div class="arf_popup_checkbox_wrapper" >
								...
								<span><label for="arf_conditional_enable_disable_mail_id_chkbox" class="arf_auto_responder_label arfhelptip" title="<?php echo __( 'Please select options to send an automatic response to user.', 'arforms-form-builder' ); ?>"><?php echo  __( 'Configure Conditional Email Notification', 'arforms-form-builder' ) ; ?><span class="arflite_pro_version_notice">(Premium)</span></label></span>
								</div>
							</div>
						...
						<div class="arf_auto_responder_row">
							<div class="arf_auto_responder_column">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'From/Replyto Name', 'arforms-form-builder' ) ; ?></label>
								<input type="text" id="options_ar_user_from_name" name="options[ar_user_from_name]" value="<?php echo ( isset( $values['ar_user_from_name'] ) && $values['ar_user_from_name'] != '' ) ? esc_attr($values['ar_user_from_name']) : esc_attr($arflitesettings->reply_to_name); ?>" <?php echo ( isset( $values['auto_responder'] ) && $values['auto_responder'] < 1 ) ? ' disabled="disabled" ' : ''; ?> >
							</div>

							<div class="arf_auto_responder_column">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'From E-mail', 'arforms-form-builder' ) ; ?></label>
								<?php
								...
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_user_email')" id="add_field_user_email_but" <?php echo ( isset( $values['auto_responder'] ) && $values['auto_responder'] < 1 ) ? ' disabled="disabled" ' : ''; ?>> <?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;
									<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" />
								</button>
						...
						<div class="arf_auto_responder_row">
							<div class="arf_auto_responder_column">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Reply to E-mail', 'arforms-form-builder' ) ; ?></label>
								<?php
								...
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_user_nreplyto_email')" id="add_field_user_nreplyto_email_but" <?php echo ( isset( $values['auto_responder'] ) && $values['auto_responder'] < 1 ) ? ' disabled="disabled" ' : ''; ?>><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;
									<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" />
								</button>
						...
						<div class="arf_auto_responder_row">
							<div class="arf_width_80">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Message', 'arforms-form-builder' ) ; ?></label>
								<?php
								...
								?>
								<span class="arferrmessage arferrmessageselectpage" id="ar_email_message_error"><?php echo  __( 'This field cannot be blank', 'arforms-form-builder' ) ; ?></span>
								<textarea class="arf_email_message_text" name="options[ar_email_message]" id="ar_email_message_text"><?php echo $ar_email_message; ?></textarea>
							</div>
							<div class="arf_width_20">
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_message')" id="add_field_message_but" <?php echo ( isset( $values['auto_responder'] ) && $values['auto_responder'] < 1 ) ? ' disabled="disabled" ' : ''; ?> ><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;
									<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" />
								</button>
							...
							<div class="email-msg-shortcode">
								<div><label><code>[ARFLite_form_all_values]</code> - <?php echo  __( 'This will be replaced with form\'s all fields & labels.', 'arforms-form-builder' ) ; ?></label></div>
								<div><label><code>[ARFLite_form_referer]</code> - <?php echo __( 'This will be replaced with entry referer.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_form_added_date_time]</code> - <?php echo __( 'This will be replaced with entry added time.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_form_ipaddress]</code> - <?php echo __( 'This will be replaced with IP Address.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_form_browsername]</code> - <?php echo __( 'This will be replaced with user browser name.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_form_entryid]</code> - <?php echo __( 'This will be replaced with Entry ID.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_current_userid]</code> - <?php echo __( 'This will be replaced with current login ID.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_current_username]</code> - <?php echo __( 'This will be replaced with current login user name.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_current_useremail]</code> - <?php echo __( 'This will be replaced with current login user email.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_page_url]</code> - <?php echo __( 'This will be replaced with current form\'s page URL.', 'arforms-form-builder' ); ?></label></div>

								<?php do_action( 'arflite_add_auto_response_mail_shortcode_in_out_side', $id, $values ); ?>
							</div>
						</div>
					</div>
					...
					<div class="arf_popup_checkbox_wrapper">
						...
						<span><label id="arf_admin_notification" for="chk_admin_notification" class="arffont16"><?php echo __( 'Send an automatic response to admin user after form submission.', 'arforms-form-builder' ); ?></label></span>
						</div>
					</div>
					...
					<div class="arf_admin_notification_content arfmarginl10" style="width:100%;">
						<div class="arf_auto_responder_row ">
							<div class="arf_auto_responder_column">
								...
								?>
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Admin E-mail', 'arforms-form-builder' ) ; ?></label>
								<input type="text" name="options[reply_to]" id="options_admin_reply_to_notification" value="<?php echo esc_html( $ar_admin_to_email ); ?>" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" />
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_email_to')" id="add_field_admin_email_but_to"  <?php echo $chk_admin_notification_disabled; ?> ><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
								<div class="arf_main_field_modal">
							...
							<div class="arf_auto_responder_column">
								...
								?>
								<input type="text" name="options[admin_email_subject]" id="admin_email_subject" value="<?php echo esc_attr($admin_email_subject_value); ?>" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" />
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_email_subject')" id="add_field_admin_email_but_subject"  <?php echo $chk_admin_notification_disabled; ?>><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
								<div class="arf_main_field_modal">
								...
								<div style="margin-top: 5px;">
									<div><label><code>[form_name]</code> - <?php echo  __( 'This will be replaced with form name.', 'arforms-form-builder' ) ; ?></label></div>
									<div><label><code>[site_name]</code> - <?php echo  __( 'This will be replaced with name of site.', 'arforms-form-builder' ) ; ?></label></div>
								</div>
							</div>
						</div>
						<div class="arf_auto_responder_row">
						   <div class="arf_auto_responder_column">
								...
								?>
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Admin CC Email', 'arforms-form-builder'  ); ?></label>
								<input type="text" name="options[admin_cc_email]" id="options_admin_cc_email_notification" value="<?php echo esc_html( $ar_admin_cc_email ); ?>" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" />
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_cc_email')" id="add_field_admin_cc_email_but_to"  <?php echo $chk_admin_notification_disabled; ?> ><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
								<div class="arf_main_field_modal">
						   ...
						   <div class="arf_auto_responder_column">
								...
								?>
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Admin BCC Email', 'arforms-form-builder' ) ; ?></label>
								<input type="text" name="options[admin_bcc_email]" id="options_admin_bcc_email_notification" value="<?php echo esc_html( $ar_admin_bcc_email ); ?>" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" />
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_bcc_email')" id="add_field_admin_bcc_email_but_to"  <?php echo $chk_admin_notification_disabled; ?> ><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
								<div class="arf_main_field_modal">
						...
						<div class="arf_auto_responder_row">
							<div class="arf_auto_responder_column">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'From/Replyto Name', 'arforms-form-builder' ) ; ?></label>
								<input type="text" id="options_ar_admin_from_name" name="options[ar_admin_from_name]" value="<?php echo ( isset( $values['ar_admin_from_name'] ) && $values['ar_admin_from_name'] != '' ) ? esc_attr($values['ar_admin_from_name']) : esc_attr($arflitesettings->reply_to_name); ?>" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" >
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_from_name')" id="add_field_admin_from_but_name"  <?php echo $chk_admin_notification_disabled; ?>><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
								<div class="arf_main_field_modal">
							...
							<div class="arf_auto_responder_column">
								...
								?>
								<input type="text" value="<?php echo esc_html( $ar_admin_from_email ); ?>" id="ar_admin_from_email" name="options[ar_admin_from_email]" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" />
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_email')" id="add_field_admin_email_but"  <?php echo $chk_admin_notification_disabled; ?>><?php echo  __( 'Add Field', 'arforms-form-builder' ) ; ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
								<div class="arf_main_field_modal">
						...
						<div class="arf_auto_responder_row">
							<div class="arf_auto_responder_column">
								...
								<input type="text" value="<?php echo esc_html( $ar_admin_reply_to_email ); ?>" id="ar_admin_reply_to_email" name="options[ar_admin_reply_to_email]" <?php echo $chk_admin_notification_disabled; ?> class="arf_advanceemailfield" />

								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_nreplyto_email')" id="add_field_admin_nreplyto_email_but" <?php echo $chk_admin_notification_disabled; ?>><?php echo  __( 'Add Field', 'arforms-form-builder' ); ?>&nbsp;&nbsp;
									<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" />
								</button>
						...
						<div class="arf_auto_responder_row">
							<div class="arf_width_80">
								<label class="arf_auto_responder_label_full"><?php echo  __( 'Admin Message', 'arforms-form-builder' ) ; ?></label>
								<div>
								<?php
							...
							<div class="arf_width_20">
								<button type="button" class="arf_add_field_button" onclick="arflite_add_field_fun('add_field_admin_message')" id="add_field_admin_message_but"  <?php echo $chk_admin_notification_disabled; ?> ><?php echo  __( 'Add Field', 'arforms-form-builder'  ); ?>&nbsp;&nbsp;<img src="<?php echo ARFLITEIMAGESURL; ?>/down-arrow.png" align="absmiddle" /></button>
							...
							<span class="arferrmessage" id="ar_admin_email_message_error" style="top:0px;"><?php echo  __( 'This field cannot be blank', 'arforms-form-builder' ) ; ?></span>
							<div class="arflite-clear-float email-msg-shortcode">
								...
								<div><label><code>[ARFLite_current_useremail]</code> - <?php echo __( 'This will be replaced with current login user email.', 'arforms-form-builder' ); ?></label></div>
								<div><label><code>[ARFLite_page_url]</code> - <?php echo __( 'This will be replaced with current form\'s page URL.', 'arforms-form-builder' ); ?></label></div>
								<?php do_action( 'arflite_add_admin_mail_shortcode_in_outside', $id, $values ); ?>
							</div>
						</div>
					</div>

					<?php do_action( 'arflite_additional_autoresponder_settings', $id, $values ); ?>
					<?php do_action( 'arflite_after_autoresponder_settings_container', $id, $values ); ?>
				</div>


				<div class="arf_popup_container_footer">
					<button type="button" class="arf_popup_close_button" data-id="arf_mail_notification_popup_button" ><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
				</div>
			</div>
		</div>
		...
		<div class="arf_modal_overlay">
			<div id="arf_conditional_logic_model" class="arf_popup_container arf_popup_container_conditional_logic_model" style="">
				<div class="arf_popup_container_header"><?php echo  __( 'Conditional Rule', 'arforms-form-builder' ); ?>
				...
				<div class="arf_popup_container_footer">
					<button type="button" class="arf_popup_close_button" data-id="arf_optin_popup_button" ><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
				</div>
		...
		<div class="arf_modal_overlay">
			<div id="arf_submit_action_model" class="arf_popup_container arf_popup_container_submit_action_model">
				<div class="arf_popup_container_header"><?php echo  __( 'Submit Action', 'arforms-form-builder' ) ; ?>
				...
				<div class="arf_popup_content_container arf_submit_action_container">
					...
					<p class="arftitle_p">
						<label for="conditional_logic_arfsubmit"><?php echo  __( 'Form submission action', 'arforms-form-builder' ) ; ?></label>
					...
					<div class="arf_submit_action_options">
						<div class="arf_radio_wrapper">
							...
							<span>
								<label id="success_action_message" for="success_action_message"><?php echo  __( 'Display a Message', 'arforms-form-builder' ) ; ?></label>
							</span>
						</div>
						<div class="arf_radio_wrapper">
							<div class="arf_custom_radio_div">
							...
							<span>
								<label id="success_action_redirect" for="success_action_redirect"><?php echo __( 'Redirect to URL', 'arforms-form-builder' ); ?></label>
							</span>
						</div>
						<div class="arf_radio_wrapper">
							<div class="arf_custom_radio_div" >
							...
							<span>
								<label id="success_action_page" for="success_action_page"><?php echo __( 'Display content from another page', 'arforms-form-builder' ); ?></label>
							</span>
						</div>
					</div>
					...
					<div id="arf_success_action_message" class="arf_optin_tab_inner_container arfmarginl15 arf_submit_action_inner_container <?php echo ( $values['success_action'] == 'message' ) ? 'arfactive' : ''; ?>">
						<div class="arfcolumnleft arfsettingsubtitle"><label for="success_msg" class="arf_dropdown_autoresponder_label"><?php echo  __( 'Confirmation Message', 'arforms-form-builder' ) ; ?></label></div>
						<div class="arfcolumnright fix_height">
							...
							<span class="arferrmessage" id="success_msg_error"><?php echo  __( 'This field cannot be blank', 'arforms-form-builder' ) ; ?></span>
						</div>
					</div>
					...
					<div id="arf_success_action_redirect" class="arf_optin_tab_inner_container arfmarginl15 arf_submit_action_inner_container <?php echo ( $values['success_action'] == 'redirect' ) ? 'arfactive' : ''; ?>">
						<label for="success_url" class="arf_dropdown_autoresponder_label"><?php echo __( 'Set Static Redirect URL', 'arforms-form-builder' ); ?></label>
						...
						<span class="arferrmessage" id="success_url_error" style='top:0;'><?php echo  __( 'This field cannot be blank', 'arforms-form-builder'  ); ?></span>
						<br/><i class="arf_notes redirect-urlnot"><?php echo __( 'Please insert url with http:// or https://.', 'arforms-form-builder' ); ?></i>
						<?php do_action( 'arflite_form_submit_after_redirect_to_url', $id, $values ); ?>
						...
						<div class="arfcolumnleft arf_custom_margin_redirect arfsetcondtionalredirect">
							<div class="arf_custom_checkbox_div">
								...
								<span>
									<label for="arf_sa_data_with_url" style="margin-left: 4px;"><?php echo  __( 'Send form data to redirected page/post', 'arforms-form-builder'  ); ?><span class="arflite_pro_version_notice">(Premium)</span></label>
								</span>
							</div>
						</div>
					</div>
					...
					<div id="arf_success_action_page" class="arf_optin_tab_inner_container arfmarginl15 arf_submit_action_inner_container <?php echo ( $values['success_action'] == 'page' ) ? 'arfactive' : ''; ?>">
						<div class="arf_ar_dropdown_wrapper">
							<label class="arf_dropdown_autoresponder_label" id="arf_use_content_from_page"><?php echo  __( 'Select Page', 'arforms-form-builder'  ); ?></label>
							...
							<span class="arferrmessage arferrmessageselectpage" id="option_success_page_id_error"><?php echo  __( 'This field cannot be blank', 'arforms-form-builder' ) ; ?></span>
						</div>
					</div>
					...
					<div class="arf_popup_checkbox_wrapper arf_hide_form_sub arflite-hideaftersave-wrap">
						<div class="arf_custom_checkbox_div">
							...
							<span><label id="arf_hide_form_after_submitted" for="arf_hide_form_after_submitted"><?php echo  __( 'Hide Form after submission', 'arforms-form-builder' ) ; ?></label></span>
						</div>
					</div>

					<?php do_action( 'arflite_option_before_submit_conditional_logic', $id, $values ); ?>
						...
						<div class="field_conditional_law field_basic_option arf_fieldoptiontab" style="display:block;">
							<div class="arf_enable_conditional_submit_div" <?php echo( is_rtl() ) ? 'style="margin-right:1px;"' : ''; ?>>
								<div class="arf_custom_checkbox_div">
									...
									<span>
										<label for="conditional_logic_arfsubmit" class="arftitle_p" style="margin-left: 4px;font-size: 16px !important; margin-top: 3px;"><?php echo  __( 'Configure conditional submission', 'arforms-form-builder'  ); ?><span class="arflite_pro_version_notice">(Premium)</span></label>
									</span>
								</div>
							</div>
						</div>
					...
					<?php do_action( 'arflite_after_onsubmit_settings_container', $id, $values ); ?>
				</div>
				<div class="arf_popup_container_footer">
					<button type="button" class="arf_popup_close_button" data-id="arf_submit_popup_button" ><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
				</div>
			</div>
		</div>
		...
		<div class="arf_modal_overlay">
			<div id="arf_optin_model" class="arf_popup_container arf_popup_container_option_model">
				<div class="arf_popup_container_header"><?php echo  __( 'Opt-ins (email marketing) configuration', 'arforms-form-builder'  ); ?>
					<div class="arfpopupclosebutton arfmodalclosebutton" data-dismiss="arfmodal" data-id="arf_optin_popup_button">
						<svg width="30px" height="30px" viewBox="1 0 20 20"><g id="preview"><path fill-rule="evenodd" clip-rule="evenodd" fill="#262944" d="M10.702,10.909L6.453,6.66l-4.249,4.249L1.143,9.848l4.249-4.249L1.154,1.361l1.062-1.061l4.237,4.237l4.238-4.237l1.061,1.061L7.513,5.599l4.249,4.249L10.702,10.909z"></path></g></svg>
					</div>
				</div>
		...
		<div class="arf_modal_overlay">
			<div id="arf_other_options_model" class="arf_popup_container arf_popup_container_other_option_model">
				<div class="arf_popup_container_header"><?php echo  __( 'General Options', 'arforms-form-builder'  ); ?>
		...
		<div class="arf_modal_overlay">
			...
			<div id="arf_hidden_fields_options_model" class="arf_popup_container arf_popup_container_other_option_model">
				<div class="arf_popup_container_header"><?php echo __( 'Hidden Input Fields Options', 'arforms-form-builder' ); ?>
					<div class="arfpopupclosebutton arfmodalclosebutton" data-dismiss="arfmodal" data-id="arf_optin_popup_button">
						<svg width="30px" height="30px" viewBox="1 0 20 20"><g id="preview"><path fill-rule="evenodd" clip-rule="evenodd" fill="#262944" d="M10.702,10.909L6.453,6.66l-4.249,4.249L1.143,9.848l4.249-4.249L1.154,1.361l1.062-1.061l4.237,4.237l4.238-4.237l1.061,1.061L7.513,5.599l4.249,4.249L10.702,10.909z"></path></g></svg>
					</div>
				</div>
				...
				<div class="arf_popup_content_container arf_other_options_container">
					...
					<div class="arf_hidden_fields_wrapper">
						<span class="arf_hidden_field_title"><?php echo __( 'Hidden Input Fields Setup', 'arforms-form-builder' ); ?></span>
						<div class="arf_hidden_field_note">
							<div><?php echo  __( 'Note', 'arforms-form-builder' )  . ': ' . __( 'These fields will not shown in the form. Enter the value to be hidden', 'arforms-form-builder' ); ?></div>

							<div>[ARF_current_user_id] : <?php echo  __( 'This shortcode replace the value with currently logged-in User ID.', 'arforms-form-builder' ) ; ?></div>
							<div>[ARF_current_user_name] : <?php echo  __( 'This shortcode replace the value with currently logged-in User Name.', 'arforms-form-builder'  ); ?></div>
							<div>[ARF_current_user_email] : <?php echo  __( 'This shortcode replace the value with currently logged-in User Email.', 'arforms-form-builder'  ); ?></div>
							<div>[ARF_current_date] : <?php echo  __( 'This shortcode replace the value with current Date.', 'arforms-form-builder'  ); ?></div>

						</div>
						<button type="button" id="arf_add_new_hidden_field" class="rounded_button arf_btn_dark_blue add_new_hidden_field_button" style="<?php echo ( count( $all_hidden_fields ) > 0 ) ? 'display:none;' : ''; ?>"><?php echo  __( 'Add new hidden field', 'arforms-form-builder'  ); ?></button>
						<div class="arf_hidden_field_input_wrapper_header <?php echo ( count( $all_hidden_fields ) > 0 ) ? 'arfactive' : ''; ?>">
							<span class="arf_hidden_field_input_wrapper_header_label"><?php echo  __( 'Label', 'arforms-form-builder'  ); ?></span>
							<span class="arf_hidden_field_input_wrapper_header_value"><?php echo  __( 'Value', 'arforms-form-builder'  ); ?></span>
							<span class="arf_hidden_field_input_wrapper_header_action"><?php echo  __( 'Action', 'arforms-form-builder'  ); ?></span>
						</div>
		...
		<div class="arf_modal_overlay">
			...
			<div id="arf_tracking_code_options_model" class="arf_popup_container arf_popup_container_other_option_model ">
				<div class="arf_popup_container_header"><?php echo  __( 'Submit Tracking Script', 'arforms-form-builder'  ); ?>
					<div class="arfpopupclosebutton arfmodalclosebutton" data-dismiss="arfmodal" data-id="arf_optin_popup_button">
						<svg width="30px" height="30px" viewBox="1 0 20 20"><g id="preview"><path fill-rule="evenodd" clip-rule="evenodd" fill="#262944" d="M10.702,10.909L6.453,6.66l-4.249,4.249L1.143,9.848l4.249-4.249L1.154,1.361l1.062-1.061l4.237,4.237l4.238-4.237l1.061,1.061L7.513,5.599l4.249,4.249L10.702,10.909z"></path></g></svg>
					</div>
				</div>
				...
				<div class="arf_popup_content_container arf_other_options_container">
					...
					<div class="arf_submit_action_tab_wrapper">
						...
						<div class="arf_after_submission_tracking_code">
							 <span class="arf_hidden_field_title arf_submission-tracking-title"><?php echo  __( 'After Submission Tracking Script', 'arforms-form-builder'  ); ?></span>
							<div class="arftablerow prevent_duplicate_message_box prevent_duplicate_box arfsubmissiontrackingcontainer">
								<div class="arfcolumnleft arfsettingsubtitle"><?php echo  __( 'Enter After submission tracking script', 'arforms-form-builder'  ); ?>&nbsp;(<?php echo  __( 'Example: Google Tracking Code', 'arforms-form-builder'  ); ?>)</div>
								<div class="arfcolumnright arf_pre_dup_msg_width">
									<div class="arflite-tracking-script">&lt;script type="text/javascript"&gt;</div>
									<textarea rows="10" id="arf_submission_tracking_code" name="options[arf_sub_track_code]" class="txtmodal1 auto_responder_webform_code_area arf_submission_tracking_code"><?php echo( isset( $values['arf_sub_track_code'] ) && $values['arf_sub_track_code'] != '' ) ? rawurldecode( stripslashes_deep( $values['arf_sub_track_code'] ) ) : ''; ?></textarea><br />
									<div class="arferrmessage display-none-cls" id="arf_submission_tracking_code"><?php echo  __( 'This field cannot be blank', 'arforms-form-builder'  ); ?></div>
									<div class="arflite-tracking-script">&lt;/script&gt;</div>
								</div>
								<div class="arfsub_track-ins"><?php echo __( '(Do not insert script tag', 'arforms-form-builder' ) . '(&lt;script&gt;)' . __( ' inside code.)', 'arforms-form-builder' ); ?></div>
							</div>
						</div>
...
<div class="arf_modal_overlay">
	<div id="arf_fontawesome_model" class="arf_popup_container arf_popup_container_fontawesome_model">
		<div class="arf_popup_container_header"><?php echo  __( 'Font Awesome', 'arforms-form-builder'  ); ?></div>
		...
		<div class="arf_popup_container_footer arf_popup_container_footer_height_auto">
			...
			<button type="button" class="arf_popup_close_button arf_popup_close_button_cancel"><?php echo  __( 'Cancel', 'arforms-form-builder'  ); ?></button>&nbsp;&nbsp;
			<button type="button" class="arf_popup_close_button arf_fainsideimge_ok_button" id="" ><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
		</div>
	</div>
</div>
...
<div class="arf_modal_overlay">
	<input type="hidden" id="open_new_form_div" value="<?php echo isset( $_REQUEST['isp'] ) ? intval( $_REQUEST['isp'] ) : 0; ?>" />
...
<div class="arf_modal_overlay arf_whole_screen">
	<div id="form_previewmodal" class="arf_popup_container arf_hide_overflow">
		...
		<div class="arfmodal-body arf_hide_overflow arflite-clear-float arflite_padding0">
			<div class="iframe_loader arf_editor_preview_loader" align="center"><?php echo ARFLITE_LOADER_ICON; ?></div>
...
<div class="arf_modal_overlay">
	<div id="arf_other_css_expanded_model"  class="arf_popup_container arf_popup_container_other_css_expanded_model arf_hide_overflow">
		<div class="arf_other_css_expanded_model_header">
			<span><?php echo  __( 'Custom CSS', 'arforms-form-builder'  ); ?></span>
		...
		<div class="arf_popup_container_footer">
			<button type="button" class="arf_popup_close_button" id="arf_css_expanded_model_btn"><?php echo __( 'OK', 'arforms-form-builder' ); ?></button>
		</div>
	</div>
</div>
