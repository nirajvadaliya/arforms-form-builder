...
$arfliteversion        = '1.5.3';
$arflitedbversion      = '1.5.3';
	...
	if ( IS_WPMU ) {
		...
		if ( $blogs ) {
			foreach ( $blogs as $blog ) {
				...
				$is_pro_form_table = $wpdb->query( "SHOW TABLES LIKE '" . $pro_form_table . "' ");
				...
				if( $is_pro_form_table > 0 ){
					...
					if( !empty( $all_lite_forms ) ){
						foreach( $all_lite_forms as $lite_form ){
							...
							/* Get Entries */
							$all_lite_form_fields = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM `{$pro_entries}` WHERE form_id = %d", $lite_form_id ) );
							...
							$all_pro_incomplete_entries = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM `{$pro_incomplete_entries}` WHERE form_id = %d", $lite_form_id ) );
