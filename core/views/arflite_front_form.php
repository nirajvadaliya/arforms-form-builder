...
if ( ! function_exists( 'arflite_get_form_builder_string' ) ) {
	...
	function arflite_get_form_builder_string( $id, $key = '', $preview = 0, $is_widget_or_modal = 0, $errors = array(), $arflite_data_uniq_id = '', $desc = '', $type = '', $modal_height = '', $modal_width = '', $position = '', $btn_angle = '', $bgcolor = '', $txtcolor = '', $open_inactivity = '', $open_scroll = '', $open_delay = '', $overlay = '', $is_close_link = '', $modal_bgcolor = '', $is_fullscrn = '', $inactive_min = '', $model_effect = '', $navigation = false, $arf_preset_data = '', $hide_popup_for_loggedin_user = 'no' ) {
		...
		if ( $arflite_func_val != '' && isset( $_POST[ 'is_submit_form_' . $id ] ) ) {
		...
		if ( $params['action'] == 'create' && $params['posted_form_id'] == $form->id && isset( $_POST ) ) {
			...
			$errors = isset( $arflitecreatedentry[ $form->id ]['errors'] ) ? $arflitecreatedentry[ $form->id ]['errors'] : array();
						...
						if ( $conf_method == 'page' && is_numeric( $form_options['success_page_id'] ) ) {
							...
							if ( $form_options['success_page_id'] != $post->ID ) {
								...
								foreach ( $matches[0] as $key => $val ) {
									...
									$temp_form_opts = $wpdb->get_row( $wpdb->prepare( 'SELECT `form_css` FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $arf_form_id_extracted ) );
		...
		if ( isset( $preview ) && $preview ) {
			...
			$arf_form   .= '<form enctype="' . apply_filters( 'arfliteformenctype', 'multipart/form-data', $form ) . '" method="post" class="arfliteshowmainform ' . esc_attr($temp_calss) . ' ' . do_action( 'arfliteformclasses', $form ) . '" style="' . esc_attr($is_hide_form) . '" data-form-id="form_' . esc_attr($form->form_key) . '" ' . esc_attr($action_html) . ' novalidate="" data-id="' . esc_attr($arflite_data_uniq_id) . '" ' . $form_attr;
			if ( $type != '' ) {
		...
		$arf_form    .= $arfliteformcontroller->arflite_get_form_hidden_field( $form, $fields, $values, $preview, $is_widget_or_modal, $arflite_data_uniq_id, $form_action, $loaded_field, $type, $is_close_link, $arf_current_token );
		...
		$arf_form .= $arfliteformcontroller->arflite_get_all_field_html( $form, $values, $arflite_data_uniq_id, $fields, $preview, $errors, $inputStyle, $arf_arr_preset_data );
		...
		$arf_form = apply_filters( 'arfliteentryform', $arf_form, $form, $form_action, $errors );
