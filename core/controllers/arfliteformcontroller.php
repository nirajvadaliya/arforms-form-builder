...
class arfliteformcontroller {
	...
	function arflite_upload_image_from_admin() {
		...
		if ( $fn && isset( $_FILES['files']['name'] ) ) {
			...
			$arflitefilecontroller = new arflitefilecontroller( $_FILES['files'], false );

			if( !$arflitefilecontroller ){
                echo '<p class="error_upload">' . $arflitefilecontroller->error_message . '</p>';
                die;
            }
            ...
            $arflitefilecontroller->nonce_data = isset( $_POST['_wpnonce_arflite'] ) ? $_POST['_wpnonce_arflite'] : '';
	...
	function arflite_process_bulk_form_actions( $errors ) {
	...
	function arflite_list_form() {
		...
		return $this->arflite_display_forms_list( $params, $message, false, false, $errors );
	}
	...
	function arflitepreview( $form_key = '' ) {
		...
		$key = ( isset( $_GET['form'] ) ? sanitize_text_field( $_GET['form'] ) : ( isset( $_POST['form'] ) ? sanitize_text_field( $_POST['form'] ) : '' ) );
	...
	function arflite_display_forms_list( $params = false, $message = '', $page_params_ov = false, $current_page_ov = false, $errors = array() ) {
	...
	function arfliteupdateformbulkoption() {
		...
		if ( $action1 == 'bulk_delete' || $action2 == 'bulk_delete' ) {
			...
			if ( $wpdb->last_error != '' ) {
				...
				$totalRecord = $wpdb->get_results( $wpdb->prepare( 'SELECT COUNT(*) as total_forms FROM ' . $ARFLiteMdlDb->forms . ' ' . $where . ' ', 0 ) );
	...
	function arflite_load_form_grid_data() {
		...
		$query = $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->forms . " WHERE is_template = %d AND (status is NULL OR status = '' OR status = 'published') ORDER BY id DESC", 0 );
	...
	if ( sanitize_text_field( $str['arfaction'] ) == 'new' || sanitize_text_field( $str['arfaction'] ) == 'duplicate' ) {
		...
		if( !empty( $_POST['arfaction'] ) && sanitize_text_field( $_POST['arfaction'] ) == 'preview' ){
			$is_preview   = true;
			$form_id      = $id = intval( $str['arfmf'] );
			$arf_preview_form_data['id'] = $form_id;
		}

		$errors = apply_filters( 'arflitevalidationofcurrentform', array(), $str );
		...
		$sel_fields = $wpdb->prepare( 'SELECT id FROM ' . $ARFLiteMdlDb->fields . ' where form_id = %d', $id );
		...
		if ( isset( $changed_field_value ) && ! empty( $changed_field_value ) ) {
			$selectDeletedFields = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM `' . $ARFLiteMdlDb->fields . "` WHERE id NOT IN( '" . implode( '\',\'', $changed_field_value ) . "') AND form_id = %d", $id ) );
			$del_fields          = $wpdb->query( $wpdb->prepare( 'delete from ' . $ARFLiteMdlDb->fields . ' where form_id = %d and id NOT IN (' . implode( ',', $changed_field_value ) . ')', $id ) );
		} elseif ( empty( $changed_field_value ) ) {
			$del_fields = $wpdb->query( $wpdb->prepare( 'delete from ' . $ARFLiteMdlDb->fields . ' where form_id = %d', $id ) );
		}
		if( !$is_preview ){
			$query_results = $wpdb->query( 'update ' . $ARFLiteMdlDb->forms . " set options = '" . addslashes( maybe_serialize( $options ) ) . "', temp_fields='" . maybe_serialize( $arf_temp_fields ) . "' where id = '" . $id . "'" );
		} else {
			$arf_preview_form_data['options'] 	= $options;
		...
		$sel_fields = $wpdb->prepare( 'select * from ' . $ARFLiteMdlDb->fields . ' where form_id = %d', $str['id'] );
		...
		echo esc_html( $message ) . '^|^' . esc_html( $id ) . '^|^' . $return_json_data_final . '^|^';

		$all_fields     = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE form_id = %d', $id ) );
		...
		if ( isset( $values['fields'] ) && ! empty( $values['fields'] ) ) {
			...
			$arf_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE `form_id` = %d', $id ), ARRAY_A );
	...
	function arflite_get_form_hidden_field( $form, $fields, $values, $preview , $is_widget_or_modal, $arflite_data_uniq_id, $form_action, $loaded_field , $type, $is_close_link, $arf_current_token ) {
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="imagename_' . $form->id . '_' . $arflite_data_uniq_id . '" id="imagename_' . $form->id . '_' . $arflite_data_uniq_id . '" value="" />';

		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arfdecimal_separator" data-id="arfdecimal_separator" value="' . $arflite_decimal_separator . '" />';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arf_success_message_show_time_' . $form->id . '" data-id="arf_success_message_show_time_' . $form->id . '" value="' . $arflitesettings->arf_success_message_show_time . '" />';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arf_validate_outside_' . $form->id . '" data-id="arf_validate_outside_' . $form->id . '" data-validate="' . ( ( apply_filters( 'arflite_validateform_outside', false, $form ) ) ? 1 : 0 ) . '" value="' . ( ( apply_filters( 'arflite_validateform_outside', false, $form ) ) ? 1 : 0 ) . '" />';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arf_is_resetform_aftersubmit_' . $form->id . '" data-id="arf_is_resetform_aftersubmit_' . $form->id . '" value="' . ( ( apply_filters( 'arflite_is_resetform_aftersubmit', true, $form ) ) ? 1 : 0 ) . '" />';

		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arf_is_resetform_outside_' . $form->id . '" data-id="arf_is_resetform_outside_' . $form->id . '" value="' . ( ( apply_filters( 'arflite_is_resetform_outside', false, $form ) ) ? 1 : 0 ) . '" />';
		...
		if('1' == $arflitesettings->arfmainformloadjscss){
			$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="is_load_js_and_css_in_all_pages" data-id="is_load_js_and_css_in_all_pages" value="' . $arflitesettings->arfmainformloadjscss . '" />';
		}
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="form_submit_type" data-id="form_submit_type" value="' . $arflitesettings->form_submit_type . '" />';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arflite_skip_captcha" data-id="arflite_skip_captcha" value="' . $arflitesettings->hidden_captcha . '" />';
	...
	function arflite_get_all_field_html( $form, $values, $arflite_data_uniq_id, $fields, $preview, $errors, $inputStyle, $arf_arr_preset_data = array()) {
        ...
        if(false == $OFData){
            $OFData = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->fields . ' WHERE form_id = %d ORDER BY id', $form->id ) );
			...
			if ( is_array( $field ) || is_object( $field ) ) {
				...
				if ( isset( $is_confirmation_method ) && ! $is_confirmation_method || ! isset( $is_confirmation_method ) ) {
					if ( isset( $_REQUEST ) && isset( $_REQUEST['item_meta'] ) && array_key_exists( $field['id'], $_REQUEST['item_meta'] ) ) {
						$field['set_field_value'] = wp_kses( $_REQUEST['item_meta'][ $field['id'] ], $allowed_html );
					}
				}
				...
				switch ( $field['type'] ) {
					case 'text':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$return_string .= '<input ' . $arf_regular_expression . '  type="text" id="field_' . $field['field_key'] . '_' . $arflite_data_uniq_id . '" ';
					...
					case 'number':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$return_string .= '<input type="' . $num_field_type . '" class="arf_number_field" id="field_' . $field['field_key'] . '_' . $arflite_data_uniq_id . '" dir=" ';
					...
					case 'tel':
						...
						if ( isset( $field['phonetype'] ) && $field['phonetype'] == 1 ) {
							...
							$return_string .= "<input type='hidden' data-jqvalidate='false' name='" . $phone_hidden_name . "' id='field_" . $field['field_key'] . '_' . $arflite_data_uniq_id . "_country_code' />";
							...
							$return_string .= "<input type='hidden' data-jqvalidate='false' id='field_" . $field['field_key'] . '_' . $arflite_data_uniq_id . "_default_country' value='" . $arf_default_country . "' />";
						}
						$return_string .= '</div>';
						break;
					case 'url':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$return_string .= '<input type="url" id="field_' . $field['field_key'] . '_' . $arflite_data_uniq_id . '" ';
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							if( '' == $defaultdate_format ){
								...
								if ( $formate == 'MM/DD/YYYY' ) {
									...
									$formate = 'd/m/Y';
								} else if($formate == 'D.MM.YYYY'){
						            $formate = 'd.m.Y';
						        } else if($formate == 'D.MMMM.YY'){
						            $formate = 'd.F.y';
						        } else if($formate == 'YYYY.MM.D'){
						            $formate = 'Y.m.d';
						        } else if($formate == 'D. MMMM YYYY'){
						            $formate = 'd. F Y';
								}
					...
					case 'image':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$return_string .= '<input type="url" id="field_' . $field['field_key'] . '_' . $arflite_data_uniq_id . '" name="' . $field_name . '" ';
							if ( isset( $field['set_field_value'] ) ) {
	...
	function arflite_change_input_style() {
		...
		$form_id      = $id = isset( $_POST['form_id'] ) ? intval( $_POST['form_id'] ) : '';
		$style        = isset( $_POST['style'] ) ? sanitize_text_field( $_POST['style'] ) : 'material';
		...
		$arf_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE `form_id` = %d', $form_id ), ARRAY_A );
		asort( $field_order );
		$arf_sorted_fields = array();
		foreach ( $field_order as $field_id => $order ) {
			if ( is_int( $field_id ) ) {
				...
				foreach( $fval as $fki => $fvali ){
					if( 'name' == $fki || 'description' == $fki || 'arf_tooltip' == $fki || 'tooltip_text' == $fki ){
						...
						foreach( $fvali_arr as $fopt_k => $fopt_v ){
							...
							} else if( in_array( $fopt_k, $arflite_intval_keys ) ){
								$fvali_arr[ $fopt_k ] = intval( $fopt_v );
							} else if( $fopt_k == 'phtypes' ){
					...
					} else if ( in_array( $fki, $arflite_intval_keys ) ){
						...
						if( is_array( $fopt_v ) ){
							$fvali_arr[ $fopt_k ] = array_map( 'sanitize_text_field', $fopt_v );
						} else {
		...
		if ( is_array( $styling_opts ) && ! empty( $styling_opts ) && count( $styling_opts ) > 0 ) {
			...
			$form_css['arf_bg_position_input_y'] = ( isset( $styling_opts['arf_bg_position_input_y'] ) && $styling_opts['arf_bg_position_input_y'] != '' ) ? intval( $_REQUEST['arf_bg_position_input_y'] ) : '';
	...
	function arflite_load_form_css( $form_id, $inputStyle ) {
		...
		if ( $arflitesettings->arfmainformloadjscss != 1 && ! wp_style_is( $stylesheet_handler, 'enqueued' ) ) {
	...
	function arflite_replace_default_value_shortcode_func( $default_value, $field, $form ) {
		...
		$pattern2 = '/(\[arf_current_user_detail(\s+)key\=(\'|\")(.*?)(\'|\")\])/';

		preg_match_all( $pattern, $default_value, $matches );

		preg_match_all( $pattern2, $default_value, $matches );

		foreach ( $matches as $parent_k => $match ) {

			$meta_key = isset( $match[0] ) ? $match[0] : '';
		
			if ( isset( $match ) && is_array( $match ) && count( $match ) == 1 && $meta_key == "user_email") {
			...
			else if (isset( $match ) && is_array( $match ) && count( $match ) == 1 && $meta_key == "site_url") {

					if ( $meta_key == '' ) {
						$default_value = preg_replace( $pattern, '', $default_value );
					} else {
						$site_url = get_site_url();
						$default_value = preg_replace( $pattern2, $site_url, $default_value );
					}
	...
	function arflite_rewrite_form_css( $form_id, $frm_css ){
        ...
        $temp_fres = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $ARFLiteMdlDb->fields . " WHERE form_id = %d ORDER BY id", $form->id));

		$loaded_field = array();
	if( empty( $temp_fres ) ) { return; }
