...
class arflite_post_value {
	...
	function arflite_post_values_after_redirect_to_url_html( $id, $values ) {?>
				...
				<div class="arf_submit_action_post_values_enable">
					<div class="arf_popup_checkbox_wrapper arflite_post_value_checkbox_wrapper">
						<div class="arf_custom_checkbox_div">
							...
							<span>
								<label for="arf_show_post_value"><?php echo __( 'Send Form Data internaly to Custom URL', 'arforms-form-builder' ); ?> (<?php echo __( 'Webhook', 'arforms-form-builder' ); ?>)<span class="arflite_pro_version_notice">(Premium)</span></label>
							</span>
						</div>
					</div>
					<span class="arf_submit_action_post_values_inner_block">
					<?php echo __( '(Upon successful submission form entry data will be sent to below mentioned url using POST method.)', 'arforms-form-builder' ); ?></span>
				</div>
			</div>
	  </div>
		<?php
